 ...
 * kind of context-object that serves as handle for a certain instance of an
 * engine; instead, clojure.lang.RT works by using thread-locals as handle. This
 * has the implication that we must manage internal handles to different engines
 * on a separate thread for each instance. In other words, the instance of
 * ClojureScriptEngine (which wraps RT) is a regular object, but internally it
 * has to communicate with a reference to RT on another thread. This is not
 * important to users of this class, I just felt a need to justify my rather
 * strange design.
 *
 * TODO: - How to retrieve a map of current bindings? Use
 * namespace-introspection? - get("a") can probably be implemented by simply
 * evaluating "a" - What's best for set("a", (Object)a)? Use Var.find and
 * Var.set?
 *
 */
public class ClojureScriptEngine extends AbstractScriptEngine
    ...
    private final Symbol instanceNS = Symbol.create("cse-" + Integer.toString(
            RT.nextID()));
    ...
    static final Var compile_path = RT.var(ClojureBindings.nsClojure,
            "*compile-path*");
    static final Var warn_on_reflection = RT.var(ClojureBindings.nsClojure,
            "*warn-on-reflection*");
    static final Var print_meta = RT.var(ClojureBindings.nsClojure,
            "*print-meta*");
    static final Var print_length = RT.var(ClojureBindings.nsClojure,
            "*print-length*");
    static final Var print_level = RT.var(ClojureBindings.nsClojure,
            "*print-level*");
    // END From clojure.lang.Repl
    private ClojureBindings engineBindings = new ClojureBindings();
    private ClassLoader clojureClassLoader = null;

    public ClojureScriptEngine(ClassLoader classLoader)
        ...
        this.clojureClassLoader = classLoader;
        ...
        submitAndGetResult(new CallableClojureInitialization(
                getAssociativeBindings(), instanceNS));
...
//        }
        ...
        return RT.map(
                ...
                clojure.lang.Compiler.LOADER, clojureClassLoader);
    }
    ...
    public Object eval(Reader reader, ScriptContext context)
            throws ScriptException {
        CallableEval c = new CallableEval(reader, context, instanceNS, clojureClassLoader);
...
class CallableEval implements Callable<Object> {
    ...
    public CallableEval(Reader reader, ScriptContext context, Symbol ns, ClassLoader cl) {
        this.reader = reader;
...
//            b.put(clojure.lang.Compiler.LOADER, dynamicClassLoader);
            Var.pushThreadBindings(RT.map(clojure.lang.Compiler.LOADER,
                    cl));
            ...
            for (;;) {
                try {
                    ...
                    ScriptException scriptException = new ScriptException(
                            "Exception in Clojure RT");
                    scriptException.initCause(
                            e instanceof Compiler.CompilerException ? e : c);
...
class CallableClojureInvokeFunction implements Callable<Object> {
    ...
    public Object call() {
        try {
            ...
            Var fnVar = (Var) Compiler.maybeResolveIn(Namespace.find(ns),
                    nameSym);

            if (fnVar == null) {
                // We could not resolve the function within the current
                // namespace, so try a broader scope
                fnVar = (Var) Compiler.maybeResolveIn(Namespace.find(ns),
            ...
            if (args != null) {
                if (args.length > 0) {
                    return fnVar.applyTo(ClojureScriptEngine.makeSeqFromArray(
                            args));
                } else {
                    return fnVar.invoke();
                }
    ...
    public Object call() {
        try {
            ...
            return Compiler.eval(
                    ...
                    new Object[]{Symbol.intern(".".intern()),
                thiz,

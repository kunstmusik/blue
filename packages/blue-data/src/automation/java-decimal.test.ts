...
describe("parseJavaDecimal (Java BigDecimal(String) grammar)", () => {
  ...
  ])("reports scale overflow for %s", (text) => {
    const result = parseJavaDecimal(text);
    expect(result.ok).toBe(false);
...
describe("javaDecimalToBinary64 (Java BigDecimal.doubleValue())", () => {
  it.each([
    ...
    [
      "2.470328229206232720882843964341106861825299013071623822127928412503377536351043E-324",
      0.0,
    ],
...
describe("setScale, remainder, subtract", () => {
  it("floors toward negative infinity", () => {
    expect(mustOk(javaDecimalSetScaleFloor(parseOk("-0.46"), 1), "-0.46").canonicalText).toBe("-0.5");
    expect(mustOk(javaDecimalSetScaleFloor(parseOk("-0.45"), 1), "-0.45").canonicalText).toBe("-0.5");
  ...
  it("half-up rounds away from zero on ties", () => {
    expect(mustOk(javaDecimalSetScaleHalfUp(parseOk("2.675"), 5), "2.675").canonicalText).toBe("2.67500");
  ...
  it("computes Java signed remainder and subtraction", () => {
    const v = mustOk(javaDecimalSetScaleFloor(mustOk(javaDecimalFromBinary64(0.549), "0.549"), 1), "0.549");

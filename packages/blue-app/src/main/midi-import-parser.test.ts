...
describe('MIDI import parser', () => {
  ...
  it('supports format 1 and splits note-bearing channels into streams', () => {
    ...
    const result = parseMidiImportBytes(buildFile(1, 960, [track, [0x00, 0xff, 0x2f, 0x00]]), 'multi.mid');

    expect(result.document.format).toBe(1);
    expect(result.document.tracks[0].streams.map((stream) => stream.streamKey)).toEqual(['0:0', '0:1']);
  });
  ...
  it('rejects SMPTE and format-2 files', () => {
    expect(() => parseMidiImportBytes(buildFile(0, 0xE728, [[0x00, 0xff, 0x2f, 0x00]]), 'smpte.mid'))
      .toThrow(/positive PPQ/);
    expect(() => parseMidiImportBytes(buildFile(2, 480, [[0x00, 0xff, 0x2f, 0x00]]), 'format-2.mid'))
      .toThrow(/formats 0 and 1/);
  });

  it('rejects malformed files without leaking parser errors', () => {
    expect(() => parseMidiImportBytes(new Uint8Array([0x00, 0x01, 0x02]), 'broken.mid'))
      .toThrow(/MIDI|Bad/);
  });
});

...
function fromServiceError(error: unknown): CodeRepositoryError {
  if (error instanceof ServiceError) {
    return createCodeRepositoryError(error.code, error.message, error.retryable, error.currentSnapshot);
  }
...
function isCreateGroupRequest(value: unknown): value is CodeRepositoryCreateGroupRequest {
  if (!isStringRecord(value)) return false;
  return isNonBlankString(value.parentId) && isNonBlankString(value.name) && isRevision(value.expectedRevision);
}
...
export function registerCodeRepositoryIpc(options: CodeRepositoryIpcOptions): () => void {
  ...
  ipcMain.handle(CODE_REPOSITORY_GET_SNAPSHOT_CHANNEL, async () => {
    ...
    if (status.diagnostic) {
      return fail(createCodeRepositoryError('storage-unavailable', status.diagnostic.message, true));
    }
  ...
  ipcMain.handle(CODE_REPOSITORY_EXPORT_XML_CHANNEL, async () => {
    ...
    try {
      ...
      return fail(
        createCodeRepositoryError('export-failed', 'Unable to write the Code Repository export.', false),
      );
    }
  });
...
export type { CodeRepositorySnapshot, CodeRepositoryStatus, CodeRepositoryDiagnostic, CodeRepositoryImportResult };

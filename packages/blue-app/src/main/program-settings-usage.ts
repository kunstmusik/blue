...
export const FEATURE_PARITY_NOTES: readonly FeatureParityNote[] = [
  {
    ...
    currentAppStatus: 'Implemented: disk render executes through the managed Blue Engine Csound runtime, validates output, and supports render-and-play/open follow-up commands.',
  ...
  },
  {
    ...
    currentAppStatus: 'Implemented: selected timeline SoundObjects freeze/unfreeze through the managed Blue Engine Csound runtime and freeze flags.',
    recommendedSpecScope: 'Implemented by SPEC 056; retain this entry only as a Java-parity usage reference.',
  },
  {
    ...
    currentAppStatus: 'Implemented: SoundFont Viewer uses the managed Blue Engine Csound runtime to inspect .sf2 instrument and preset metadata.',
  ...
  },
  {
    ...
    currentAppStatus: 'Realtime driver settings query the active Csound runtime on demand; render method is the managed Blue Engine service.',
    recommendedSpecScope: 'Implemented by SPEC 071; retain this entry only as a Java-parity usage reference.',
  },
  {
...
export function buildUsageMatrix(): UsageParityMatrixEntry[] {
  return [
    ...
    entry('realtimeRender', 'realtimeRender.audioDriver', 'Audio Module', 'pa_bl (macOS)', 'Runtime audio module selection', 'used-by-workflow', { consumerPath: 'CsoundRuntimeService query' }),
    ...
    entry('realtimeRender', 'realtimeRender.midiDriver', 'MIDI Module', 'PortMidi', 'Runtime MIDI module selection', 'used-by-workflow', { consumerPath: 'CsoundRuntimeService query' }),
    ...
    entry('realtimeRender', 'realtimeRender.renderMethod', 'Render Method (legacy)', '(managed Blue Engine)', 'Retained for downgrade compatibility; managed Blue Engine is the only active service', 'app-specific-retained'),

    entry('diskRender', 'diskRender.csoundExecutable', 'Csound Executable (legacy)', '/usr/local/bin/csound (macOS)', 'Retained for downgrade compatibility; managed Blue Engine executes Csound', 'app-specific-retained', { consumerPath: 'settings migration only' }),
    ...
    entry('diskRender', 'diskRender.renderMethod', 'Render Method (legacy)', '(managed Blue Engine)', 'Retained for downgrade compatibility; managed Blue Engine is the only active service', 'app-specific-retained'),

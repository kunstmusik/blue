...
const EMPTY_SOURCES = {
  'userInstrumentLibrary.xml':
    '<instrumentLibrary><instrumentCategory categoryName="Instrument Library" isRoot="true"/></instrumentLibrary>',
  'udoLibrary.xml':
    '<udoLibrary><udoCategory categoryName="UDO Library" isRoot="true"/></udoLibrary>',
  'effectsLibrary.xml':
    '<effectsLibrary><effectCategory categoryName="Effects Library" isRoot="true"/></effectsLibrary>',
  'soundObjectLibrary.xml':
    '<soundObjectLibrary><category categoryName="SoundObject Library"/></soundObjectLibrary>',
} as const;
...
describe('automatic Java Blue migration', () => {
  it('imports all four recognized sources independently without modifying them', async () => {
    ...
    try {
      ...
      for (const [sourcePath, bytes] of original)
  ...
  it('commits three valid sources while reporting one corrupt source', async () => {
    ...
    for (const [name, xml] of Object.entries(EMPTY_SOURCES)) {
      fs.writeFileSync(
        ...
        name === 'effectsLibrary.xml' ? '<broken' : xml,
        'utf8',
      );
    }
    const client = UnifiedLibraryRepositoryClient.openForTesting(':memory:');
    try {
      ...
      expect(report.sources.find((source) => source.libraryType === 'effect')).toMatchObject({
        status: 'failed',
      });
      expect(state.load()).toMatchObject({
        legacyMigrationState: 'completed',
        lastResultKind: 'partial',
      });
    } finally {
      await client.close();
    }
  ...
  it('creates a usable empty store and records skipped when no source exists', async () => {
    ...
    try {
      ...
      expect((await client.getSnapshot()).itemCounts).toEqual({
        instrument: 0,
        udo: 0,
        soundObject: 0,
        effect: 0,
      ...
      expect(state.load()).toMatchObject({
        legacyMigrationState: 'skipped',
        lastResultKind: 'noSources',
      });
    } finally {
      await client.close();
    }
  ...
  it('discovers standard names, imports each valid source independently, and preserves source bytes', async () => {
    ...
    const source =
      '<instrumentLibrary><instrumentCategory categoryName="Instrument Library" isRoot="true"><instrument type="blue.orchestra.GenericInstrument"><name>Pad</name><globalOrc></globalOrc><globalSco></globalSco><instrumentText>ain 0</instrumentText></instrument></instrumentCategory></instrumentLibrary>';
    ...
    try {
      ...
      expect(report.status).toBe('complete');
      expect(report.sources.find((entry) => entry.libraryType === 'instrument')).toMatchObject({
        status: 'imported',
        itemCount: 1,
      });
      expect((await client.getSnapshot()).itemCounts.instrument).toBe(1);
      expect((await client.listImportHistory())[0]).toMatchObject({
        mode: 'automatic',
        status: 'completed',
      });

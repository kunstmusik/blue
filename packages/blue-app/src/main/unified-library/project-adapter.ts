  ...
  const group = data
    ...
    .find(
      (candidate): candidate is TrackLayerGroup =>
        candidate instanceof TrackLayerGroup && candidate.getUniqueId() === trackRef.rootGroupId,
    );
...
export class UnifiedLibraryProjectAdapter {
  private readonly deleteConfirmations = new Map<
    string,
    { key: string; revision: number; expiresAt: number }
  >();
  ...
  getTimelineSoundObjectSource(
    ...
    const portable = soundObject instanceof Instance ? soundObject.getSoundObject() : soundObject;
  ...
  getBlueLiveSoundObjectSource(
    ...
    const soundObject = project.data
      .getLiveData()
      .getLiveObjectBins()
  ...
  getTrackInstrumentSource(request: TrackInstrumentClipboardRequest): TrackInstrumentSource {
    const project = this.getActiveProject();
    if (!project || project.sessionId !== request.projectSessionId) {
    ...
    const group = project.data
      ...
      .find(
        (candidate): candidate is TrackLayerGroup =>
          candidate instanceof TrackLayerGroup && candidate.getUniqueId() === request.rootGroupId,
      );
  ...
  addTimelineSoundObjectToProjectLibrary(
    ...
    if (
      ...
      definition.setSubjectiveDuration(
        TimeDuration.beats(duration.toBeats(project.data.getScore().getTimeContext())),
      );
    }
  ...
  validateTransferTarget(
    ...
    if (!project || project.sessionId !== target.projectSessionId)
      return 'The destination project changed.';
    if ((project.revision ?? 0) !== target.projectRevision)
      return 'The destination changed. Choose it again.';
    const expectedType =
      target.kind === 'orchestra' || target.kind === 'trackInstrument'
        ? 'instrument'
    ...
    if (target.kind === 'orchestra') {
      const assignmentIds = project.data
        .getArrangement()
        .getArrangement()
    ...
    if (target.kind === 'effectChain') {
      ...
      if (chain.map((entry) => entry.entryId).join(':') !== target.chainRevision)
        return 'The Effect chain changed.';
      return Number.isInteger(target.insertIndex) &&
        target.insertIndex >= 0 &&
    ...
    if (target.timeContextRevision !== String(target.projectRevision))
      return 'The Score time context changed.';
    if (!Number.isFinite(target.location.startTime) || target.location.startTime < 0)
      return 'The Score time position is invalid.';
  ...
  list(libraryType: LibraryType): LibrarySearchResult[] {
    ...
    if (libraryType === 'udo') {
        ...
        return {
          key: {
            ...
            locator: {
              ...
              ...(track
                ? { track: { rootGroupId: track.rootGroupId, trackId: track.trackId } }
                : {}),
          ...
          breadcrumb: track
            ...
            : instrumentAssignmentId
              ? [
                  'Project Orchestra',
                  `${instrumentAssignmentId} ${instrumentName ?? 'Instrument'}`,
                  'UDOs',
                ]
              : ['Project UDOs'],
      ...
      const projectUdos = project.data
        .getOpcodeList()
        .getOpcodes()
        .map((opcode, index) => createResult(opcode, index));
      const instrumentUdos = project.data
        .getArrangement()
        .getArrangement()
        .flatMap((assignment) => {
          ...
          return opcodeList
            .getOpcodes()
            .map((opcode, index) =>
              createResult(opcode, index, assignment.arrangementId, assignment.instr?.getName()),
            );
        });
      const trackUdos = project.data.getScore().flatMap((group) => {
        ...
        return group.flatMap((track) => {
          ...
          if (!opcodeList) return [];
          return opcodeList.getOpcodes().map((opcode, index) =>
            createResult(opcode, index, undefined, undefined, {
  ...
  search(query: string, typeFilter: LibraryType | 'all'): LibrarySearchResult[] {
    ...
    const types: readonly LibraryType[] =
      typeFilter === 'all' ? ['instrument', 'udo', 'soundObject', 'effect'] : [typeFilter];
    return types
      .flatMap((type) => this.list(type))
      .filter((item) => item.displayName.normalize('NFKC').toLocaleLowerCase().includes(normalized))
      .sort(
        (left, right) =>
          left.libraryType.localeCompare(right.libraryType) ||
  ...
  preview(key: LibraryItemKey): LibraryItemPreview | null {
    ...
    const entry = this.list(key.libraryType).find(
    ...
    const object =
      project.data.getSoundObjectLibrary().getObjectById(key.locator.libraryId) ??
      project.data
        .getSoundObjectLibrary()
        .findUniqueByFingerprint(key.locator.persistedFingerprint);
  ...
  saveEditorSource(
    ...
    if (key.locator.kind === 'instrument') {
      ...
      changed = project.data
        .getSoundObjectLibrary()
        .replaceObjectById(key.locator.libraryId, value);
      if (changed) {
        ...
        : key.locator.kind === 'udo'
          ...
          : key.locator.kind === 'effect'
            ...
            : (project.data
                ...
                ?.getName() ?? current.displayName);
    const nextKey: LibraryItemKey =
      key.locator.kind === 'udo'
        ? {
            ...key,
            locator: {
  ...
  createContextTarget(
    ...
    const requestedRevision =
      details.targetRevision === 'current'
        ? currentRevision
  ...
  deleteProjectItem(key: LibraryItemKey, confirmationToken: string): ProjectMutationReceipt {
    ...
    if (key.locator.kind === 'instrument') {
      ...
      for (const group of project.data.getScore())
        if (group instanceof PolyObject) removeInstances(group);
  ...
  private requireCurrentTarget(
    ...
    if (target.targetRevision !== String(project.revision ?? 0))
      throw new Error('Stale target revision');
  ...
  private resolveInsertionSource(
    ...
    if (locator.kind === 'effect') {
      ...
      const entry = chain?.find(
        (candidate) => candidate.kind === 'effect' && candidate.entryId === locator.entryId,
      );
      if (!entry || entry.kind !== 'effect') throw new Error('Project Effect not found');
      return {
        displayName: entry.name || 'Unnamed Effect',
        value: Effect.loadFromXML(Element.parse(entry.effectXml)),
      };
    }
    const library = project.data.getSoundObjectLibrary();
    const value =
      library.getObjectById(locator.libraryId) ??
  ...
  private insertResolvedSource(
          ...
          objects: [
            {
              ...
              durationBeats: sound
                ...
                .toBeats(project.data.getScore().getTimeContext()),
    ...
    switch (input.key.libraryType) {
      case 'instrument': {
        ...
        const id = arrangement.addInstrumentAtIndex(instrument, insertIndex, assignmentId);
        return id;
      }
      case 'udo': {
      ...
      case 'effect': {
        if (
          ...
          !input.target.chain ||
          input.target.insertIndex === undefined
        ) {
      ...
      case 'soundObject': {
        ...
        const selectionId = randomUUID();
        const durationBeats = soundObject
          .getSubjectiveDuration()
          .toBeats(project.data.getScore().getTimeContext());
        const changed = applyProjectDocumentPatch(project.data, {
          score: {
  ...
  private resolveNewSharedInstance(
    data: BlueData,
    libraryId: string,
...
function isInstrumentValue(
  value: Instrument | OpcodeDefinition | Effect | SoundObject,
): value is Instrument {
  return (
    value instanceof GenericInstrument ||

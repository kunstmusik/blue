...
describe('legacy export compatibility', () => {
  it('exports all four roots and preserves unsupported payload XML exactly', async () => {
    ...
    const raw =
      '<soundObject type="future.Unknown"><name>Raw</name><!--keep--><plugin><![CDATA[data()]]></plugin></soundObject>';
    fs.writeFileSync(
      sourcePath,
      `<soundObjectLibrary><category categoryName="SoundObject Library">${raw}</category></soundObjectLibrary>`,
      'utf8',
    );
    ...
    try {
      ...
      await service.executeManualImport(
  ...
  it('serializes current and all-library exports through one interchange lease', async () => {
    ...
    try {
      ...
      await current;
      expect(
        parseLegacyLibraryDocument(fs.readFileSync(path.join(directory, 'udoLibrary.xml'), 'utf8'))
          .libraryType,
      ).toBe('udo');
    } finally {
      await client.close();

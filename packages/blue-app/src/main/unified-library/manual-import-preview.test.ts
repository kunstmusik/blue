...
describe('manual import preview', () => {
  ...
  it('requires an explicit stable folder identity when duplicate names make a path ambiguous', async () => {
    ...
    fs.writeFileSync(sourcePath, '<udoLibrary><udoCategory categoryName="UDO Library" isRoot="true"><udoCategory categoryName="Shared"><udo><opcodeName>tone</opcodeName><outTypes>a</outTypes><inTypes>ak</inTypes><codeBody>aout tone ain,k</codeBody><comments/></udo></udoCategory></udoCategory></udoLibrary>', 'utf8');
    const client = UnifiedLibraryRepositoryClient.openForTesting(':memory:');
    try {
      const { first, second } = await client.runForTesting((repository) => {
        const root = repository.getRoot('udo');
        return {
          first: repository.createFolder({ libraryType: 'udo', parentId: root.id, displayName: 'Shared' }),
          second: repository.createFolder({ libraryType: 'udo', parentId: root.id, displayName: 'Shared' }),
        };
      });
      const service = new UnifiedLibraryImportExportService(client);
      ...
      await expect(service.executeManualImport(blockedPreview.previewToken)).rejects.toThrow(/choose a destination/i);
      ...
      expect((await client.listChildren(second.id)).map((node) => node.displayName)).toEqual(['tone']);
    } finally {
      await client.close();
      fs.rmSync(directory, { recursive: true, force: true });
    }

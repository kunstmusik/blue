...
import { BlueData, External, PolyObject, GenericScore } from '@blue/data';
...
function makeProjectWithExternal(
  commandLine: string,
  text: string,
): { data: BlueData; request: ScoreObjectEditorRequest } {
...
describe('External test pipeline (BlueData → document → executor)', () => {
  ...
  it(
    'executes external test using document data',
    async () => {
      const { data, request } = makeProjectWithExternal(
        'node -e "process.stdout.write(\'i1 0 1\')"',
        '',
      );
      const doc = createScoreObjectEditorDocument(data, request);
      expect(doc).not.toBeNull();
  ...
  it(
    'executes external test with text body piped via cat',
    async () => {
      const { data, request } = makeProjectWithExternal(
  ...
  it(
    'mirrors Java Blue flow: command with text but $infile substitution',
    async () => {
      const { data, request } = makeProjectWithExternal(
  ...
  it(
    'generates notes from External object and stringifies NoteList (parity with main process IPC)',
    () => {

...
describe('main-process IPC inventory oracle', () => {
  it('executes the full 190-endpoint composition in startup order with exact teardown', async () => {
    ...
    try {
      ...
      expect(ipcMain.registrations).toHaveLength(190);
      expect(new Set(ipcMain.registrations).size).toBe(190);
  ...
  it('executes all 125 domain registrations in the documented baseline order and tears them down exactly', () => {
    const ipcMain = new CapturingIpcMain();
    const collected = collectedDomainHandlers();
  ...
  it('captures the current 190 inbound endpoint surface and registration modes', async () => {
    ...
    expect(collected).toHaveLength(123);
    expect(collectedInvoke).toHaveLength(120);
    expect(collectedListeners).toHaveLength(3);
    expect(collectedExpandedCount).toBe(125);
    expect(domainChannels).toHaveLength(125);
    expect(new Set(domainChannels).size).toBe(125);
    ...
    expect(domainChannels.length + existing.length).toBe(190);
  });

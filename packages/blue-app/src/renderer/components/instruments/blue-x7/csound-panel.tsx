    ...
    <div className="rounded border border-blue-border bg-blue-surface/40 p-3 space-y-3" data-testid="bluex7-csound-panel">
      ...
      <div className="flex items-center justify-between border-b border-blue-border pb-2">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-gray-200 uppercase tracking-wider">Csound & Code</span>
          <div className="flex rounded border border-blue-border bg-blue-bg/80 p-0.5 text-xs">
            <button
              type="button"
      ...
      {activeTab === 'preview' && (
        <div className="space-y-2" data-testid="bluex7-preview-tab">
          {previewError ? (
            <div className="rounded border border-red-500/50 bg-red-900/30 p-2 text-xs text-red-200" data-testid="csound-preview-error">
              {previewError}
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
              <div>
                <span className="text-[11px] font-semibold text-blue-muted block mb-1">Generated F-Tables:</span>
                <pre
                  data-testid="csound-tables-preview"
                  className="max-h-60 overflow-y-auto rounded border border-blue-border bg-blue-bg/90 p-2 font-mono text-[11px] text-gray-200"
                >
                  {preview?.tables || '; Generating tables...'}
                </pre>
              </div>
              <div>
                <span className="text-[11px] font-semibold text-blue-muted block mb-1">Generated Instrument Body:</span>
                <pre
      ...
      {activeTab === 'bindings' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3" data-testid="bluex7-bindings-tab">
          ...
          <div className="rounded border border-blue-border bg-blue-bg/40 p-2.5 space-y-1.5">
            ...
            <ul className="text-[11px] text-blue-muted space-y-1 list-disc list-inside">

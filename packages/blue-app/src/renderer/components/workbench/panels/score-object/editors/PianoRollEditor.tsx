...
import PianoRollRulerConfigDialog, {
  type PianoRollRulerConfigChanges,
} from './pianoroll/PianoRollRulerConfigDialog';
...
export default function PianoRollEditor({
  ...
  const {
    noteTemplate,
    pchGenerationMethod,
    pixelSecond: initialPixelSecond,
    noteHeight: initialNoteHeight,
  ...
  const snapBeats = useMemo(
    () => getSnapBeats(snapEnabled, snapValue, pixelSecond),
    [snapEnabled, snapValue, pixelSecond],
  );
  const effectivePrimaryTimeDisplay = payload.useGlobalRuler
    ? scoreTimeState.primaryTimeDisplay
    : payload.primaryTimeDisplay;
  const effectiveSecondaryTimeDisplay = payload.useGlobalRuler
    ? scoreTimeState.secondaryTimeDisplay
    : payload.secondaryTimeDisplay;
  const effectiveSecondaryRulerEnabled = payload.useGlobalRuler
    ? scoreTimeState.secondaryRulerEnabled
    : payload.secondaryRulerEnabled;
  ...
  const commitUndoablePatch = useCallback(
    (nextPatch: Record<string, unknown>, label: string) => {
  ...
  const handleAddNote = useCallback(
    (start: number, scaleDegree: number, octave: number, duration = 1) => {
      const fieldValues = fieldDefinitions.map((fd) => fd.defaultValue);
      commitUndoablePatch(
        {
          pianoRollNoteBatch: {
            operations: [
              { kind: 'add', note: { octave, scaleDegree, start, duration, fieldValues } },
            ],
          },
        },
        'Add note',
  ...
  const handleCommitNoteTimeEdit = useCallback(
    (_originalData: NoteData[], endData: NoteData[]) => {
      commitUndoablePatch(
        {
          pianoRollNoteBatch: {
            operations: endData.map((end) => {
              return {
                ...
                note: {
                  ...
                  duration: end.originDuration,
                  octave: end.octave,
                  scaleDegree: end.scaleDegree,
                },
              };
            }),
          },
  ...
  const handleCommitFieldValues = useCallback(
    (noteIndices: number[], fieldIndex: number, values: number[]) => {
  ...
  const handleCommitFieldEdit = useCallback(
    (noteIndices: number[], fieldIndex: number, _originalValues: number[], endValues: number[]) => {
  ...
  const handleRemoveSelectedNotes = useCallback(() => {
    ...
    commitUndoablePatch(
      {
        pianoRollNoteBatch: { operations: [{ kind: 'remove', noteIndices: [...selectedIndices] }] },
      },
      'Remove notes',
    );
    setSelectedIndices(new Set());
  ...
  const handlePasteNotesAt = useCallback(
    (startBeat: number, octave: number, scaleDegree: number) => {
  ...
  const handleRulerConfigApply = useCallback(
    (changes: PianoRollRulerConfigChanges) => {
      patch({ ...changes });
    },
    [patch],
  );
  ...
  useEffect(() => {
    ...
    selectedFieldDef,
    handleAddNote,
    handleCommitNoteTimeEdit,
    handleCommitFieldEdit,
    handleRemoveSelectedNotes,
    handleCopySelectedNotes,
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      const totalRows =
        pchGenerationMethod === GENERATE_MIDI ? MIDI_NOTE_COUNT : OCTAVES * numDegrees;
            ...
            <button
              className={cn(
                ...
                activeTab === 'notes'
                  ...
                  : 'text-blue-muted hover:text-gray-200',
              )}
              ...
              <SplitPane
                ...
                first={
                  <div className="flex flex-col h-full overflow-hidden">
                    <div
                      className="flex shrink-0 border-b border-blue-border/30 bg-app-surface-strong"
                      style={{ height: rulerHeight }}
                    >
                      <div
                        className="shrink-0 border-r border-blue-border/25"
                        style={{ width: PITCH_HEADER_WIDTH, height: rulerHeight }}
                      />
                      <div
                    ...
                    <div className="flex flex-1 min-h-0 overflow-hidden">
                      <div
                        className="relative shrink-0 overflow-hidden border-r border-blue-border/25"
                        style={{ width: PITCH_HEADER_WIDTH }}
                      >
                        <div
                          className="absolute left-0 top-0"
                ...
                second={
                  <div className="flex h-full min-h-0 bg-app-bg">
                    <div
                      className="h-full shrink-0 border-r border-blue-border/25"
                      style={{ width: PITCH_HEADER_WIDTH }}
                    >
                      <FieldSelectorView
                        fieldDefinitions={fieldDefinitions}
                    ...
                    <div
                      className="relative h-full flex-1 overflow-hidden bg-app-canvas"
                      ref={laneViewportRef}
                    >
                      <div
      ...
      <PopoutContextMenuPortal>
        <ContextMenu.Content className="editor-context-menu" {...portalEventIsolationProps}>
          <ContextMenu.Item
            ...
            onSelect={handleCopySelectedNotes}
          >
            Copy
          </ContextMenu.Item>
          <ContextMenu.Item className="editor-context-menu__item" onSelect={handleCutSelectedNotes}>
            Cut
          </ContextMenu.Item>
          <ContextMenu.Item
            className="editor-context-menu__item"
            onSelect={handlePasteAtLastTarget}
          >
            Paste
          </ContextMenu.Item>
          ...
          <ContextMenu.Item
            className="editor-context-menu__item"
            onSelect={handleRemoveSelectedNotes}
          >
            Remove
          </ContextMenu.Item>
          <ContextMenu.Item
            className="editor-context-menu__item"
            onSelect={() => setSelectedIndices(new Set(notes.map((_, i) => i)))}
          >
            Select All
          </ContextMenu.Item>
        </ContextMenu.Content>

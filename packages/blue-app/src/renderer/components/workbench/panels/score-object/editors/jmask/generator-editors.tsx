...
export function RandomEditor({
  ...
  const update = useCallback(
    (patch: Partial<GeneratorSnapshot>) => {
    ...
    <div className="flex flex-col gap-1 px-2 py-1.5">
      ...
      <div className="flex items-center gap-2">
        <CommitNumberField
          label="Min"
          value={min}
          onChange={(v) => update({ min: Math.min(v, max) })}
        />
        <CommitNumberField
          label="Max"
          value={max}
          onChange={(v) => update({ max: Math.max(v, min) })}
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-1 px-2 py-1.5">
      ...
      <div className="max-h-24 overflow-auto border border-blue-border bg-black">
        <table className="w-full text-role-body">
          <tbody>
            ...
            {listItems.length === 0 && (
              <tr>
                <td className="px-1 py-1 text-blue-muted">No items</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="flex items-center gap-1">
        <button
          type="button"
          className="px-2 py-0.5 text-role-body text-gray-300 border border-blue-border rounded hover:border-blue-accent flex items-center justify-center"
          onClick={pushUp}
        ...
        <button
          ...
          className="px-2 py-0.5 text-role-body text-gray-300 border border-blue-border rounded hover:border-blue-accent flex items-center justify-center"
          onClick={pushDown}
          disabled={selectedIdx < 0 || selectedIdx >= listItems.length - 1}
          title="Push Down"
...
export function OscillatorEditor({
  gen,
  onChange,
  duration,
}: {
    ...
    <div className="flex flex-col gap-1 px-2 py-1.5">
      ...
      <div className="flex items-center gap-3">
        <CommitNumberField
          ...
          min={0}
          max={1}
          onChange={(v) => update({ phaseInit: v })}
        />
        <CommitNumberField
          label="Exponent"
          value={exponent}
          step={0.1}
          onChange={(v) => update({ exponent: v })}
        />
      </div>
      <ConstantOrTable
        label="Frequency"
        constantValue={frequency}
        tableEnabled={freqTableEnabled}
        table={freqTable}
        duration={duration}
...
export function ProbabilityEditor({
  gen,
  onChange,
  duration,
}: {
  ...
  const handleSubGeneratorChange = useCallback(
    (idx: number, newGen: GeneratorSnapshot) => {
          ...
          options={generators.map((generator, index) => ({
            ...
            label:
              typeof generator.kind === 'string'
                ? generator.kind
                : (PROBABILITY_NAMES[index] ?? `Type ${index}`),
          }))}
        />
      </div>
      {selectedGen &&
        renderProbabilitySubEditor(
          selectedGen,
...
export function renderGeneratorEditor(
  ...
  switch (kind) {
    case 'Constant':
      return <ConstantEditor gen={gen} onChange={onChange} />;
    case 'Random':
      return <RandomEditor gen={gen} onChange={onChange} />;
    case 'ItemList':
      return <ItemListEditor gen={gen} onChange={onChange} />;
    case 'Segment':
      return <SegmentEditor gen={gen} onChange={onChange} duration={duration} />;
    case 'Oscillator':
      return <OscillatorEditor gen={gen} onChange={onChange} duration={duration} />;
    case 'Probability':
      return <ProbabilityEditor gen={gen} onChange={onChange} duration={duration} />;
    default:
      return (
        <div className="px-2 py-1 text-role-body text-blue-muted">
          Unsupported generator: {kind}
        </div>
      );
  }
}

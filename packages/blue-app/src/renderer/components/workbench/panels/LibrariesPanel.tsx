...
export default function LibrariesPanel(): React.ReactElement {
  ...
  const reorderNode = (node: LibraryBrowseNode, targetIndex: number): void => {
    if (node.scope !== 'user' || node.nodeKind === 'root' || typeof node.revision !== 'number') return;
    void state.applyMutation({
      type: 'reorderNode', nodeId: node.nodeId, expectedRevision: node.revision, targetIndex,
    });
  };
    ...
    <div className="relative flex h-full min-h-0 flex-col bg-app-bg text-app-text">
      <div className="flex flex-none items-center border-b border-app-border pr-1">
        ...
        <LibraryActionsMenu
          ...
          onImportDirectory={() => { void state.selectImportDirectory(); }}
      ...
      {state.importPreview && (
        <LibraryImportDialog
          ...
          onImport={(folderSelections) => { void state.executeImport(folderSelections); }}
          onCancel={state.cancelImport}
        />
      )}
      {folderParent && (
        <div className="fixed inset-0 z-[70] grid place-items-center bg-black/45 p-4" role="dialog" aria-modal="true" aria-labelledby="library-create-folder-title">
          <form

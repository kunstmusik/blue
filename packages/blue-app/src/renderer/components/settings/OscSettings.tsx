    ...
    >
      ...
      <div className="rounded-md border border-app-border bg-app-surface p-4 text-role-body">
        <div className="mb-3 flex items-center gap-3">
          <span
            className={cn('inline-flex items-center rounded-full px-2 py-0.5', phaseClass(runtime))}
          >
            {phaseLabel(runtime)}
          </span>
          {activePort !== null && (
            <span className="text-app-text-muted">
              Active port: <strong className="text-app-text">{activePort}</strong>
            </span>
          )}
        </div>
        ...
        {usesFallback && runtime && (
          <p className="mb-2 text-app-warning">
            Port {runtime.preferredPort} was already in use; OSC is listening on fallback port{' '}
            {activePort}. The preferred port remains unchanged and will be retried on the next
            restart.
          </p>
        )}
        ...
        {diagnostic && (
          <p className="mt-2 text-app-danger">
            {diagnostic.message}
            {diagnostic.port ? ` (port ${diagnostic.port})` : ''}
          </p>
        )}
      </div>

      <div className="mt-6">
        <h3 className="mb-1 text-role-title-3 font-semibold text-app-text-strong">
          Supported OSC Messages
        </h3>
        <p className="mb-3 text-role-callout text-app-text-subtle">
          Message arguments and bundle timetags are ignored. Addresses use Java Blue-compatible
          prefix matching.
        </p>
        <div className="overflow-hidden rounded-md border border-app-border">
          <table
            className="w-full border-collapse text-left text-role-body"
            aria-label="Supported OSC messages"
          >
            ...
            {OSC_COMMAND_CATEGORIES.map((category) => (
              <tbody
                key={category}
                className="divide-y divide-app-border border-t border-app-border"
              >
                <tr className="bg-app-surface/60">
                  <th
                    scope="colgroup"
                    colSpan={2}
                    className="px-3 py-1.5 text-role-headline font-bold text-app-text-muted"
                  >
                    {category}
                  </th>
                </tr>
                {OSC_COMMAND_REGISTRY.filter((command) => command.category === category).map(
                  (command) => (

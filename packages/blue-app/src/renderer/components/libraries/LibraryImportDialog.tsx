...
export function LibraryImportDialog({
  ...
  onImport,
  onCancel,
}: LibraryImportDialogProps): React.ReactElement {
    ...
    >
      <div className="max-h-[80vh] w-full max-w-xl overflow-auto rounded border border-app-border bg-app-overlay p-4 shadow-xl">
            ...
            <li key={source.sourcePath} className="rounded border border-app-border p-2">
              ...
              {source.error ? (
                ...
                <p className="text-app-text-muted">
                  ...
                  conflict{source.aliasConflictCount === 1 ? '' : 's'}
                </p>
              )}
              {source.ambiguousFolderCount > 0 && (
                <p role="alert" className="text-amber-400">
                  Choose destinations for ambiguous folders before importing.
                </p>
              )}
        ...
        <p className="mb-3 text-role-callout text-app-text-muted">
          Exact duplicates in the chosen folder are skipped. Same-name items with different content
          receive a deterministic Imported suffix. Missing folders are created.
        </p>

...
const projectEffectNode = {
  key: {
    ...
    projectSessionId: 4,
    locator: { kind: 'effect' as const, channelId: 'channel-1', chain: 'pre' as const, entryId: 'pre-1' },
  },
  nodeId: 'project-effect-pre-1', parentId: 'project-effects', libraryType: 'effect' as const,
  scope: 'projectOwned' as const, nodeKind: 'item' as const, displayName: 'Existing',
  breadcrumb: ['Lead', 'Pre Effects'], supportStatus: 'supported' as const,
  objectType: 'Effect', revision: 'effect-hash', hasChildren: false,
};
...
beforeEach(() => {
  ...
  window.blueAPI = {
    ...window.blueAPI,
    beginLibraryDrag: vi.fn(async () => ({ ok: true as const, value: { expiresAt: Date.now() + 30_000 } })),
    ...
    cutLibraryToClipboard: vi.fn(async () => ({
      ...
      value: {
        clipboard: {
          ...
          source: { kind: 'buffer' as const, clipboardId: 'effect-buffer', libraryType: 'effect' as const },
          capturedAt: 100,
        },
...
describe('mixer Library drop targets', () => {
  ...
  it('accepts a direct effect drop into an empty effects chain', async () => {
    ...
    const transfer = createTestDataTransfer();
    transfer.setData(BLUE_LIBRARY_DRAG_MIME, JSON.stringify({
      dragSessionId: 'drag-empty-effect', libraryType: 'effect',
    }));
    ...
    expect(previewLibraryTransfer).toHaveBeenCalledWith(expect.objectContaining({
      ...
      target: {
        kind: 'effectChain', projectSessionId: 4, projectRevision: 9,
        channelId: 'channel-1', chain: 'post', insertIndex: 0, chainRevision: '',
      },
    }));
  });
  ...
  it('shows the full chain menu when right-clicking unused Effect-bin space', async () => {
    const postEnd = container.querySelector('[aria-label="Insert Effect at end of Post chain; paste a Library item here"]') as HTMLElement;
    act(() => postEnd.dispatchEvent(new MouseEvent('contextmenu', { bubbles: true, cancelable: true })));
    await act(async () => { await Promise.resolve(); });
  ...
  it('captures Mixer Effect Copy and Cut in the shared Library buffer', async () => {
    ...
    const copy = [...document.body.querySelectorAll('[role="menuitem"]')]
      .find((item) => item.textContent === 'Copy') as HTMLElement;
    act(() => copy.click());
    expect(useLibraryStore.getState().clipboard).toMatchObject({
      operation: 'copy', source: { kind: 'library', key: projectEffectNode.key },
    });

    act(() => row.dispatchEvent(new MouseEvent('contextmenu', { bubbles: true, cancelable: true })));
    await act(async () => { await Promise.resolve(); });
    const cut = [...document.body.querySelectorAll('[role="menuitem"]')]
      .find((item) => item.textContent === 'Cut') as HTMLElement;
    await act(async () => {
      cut.click();
      await Promise.resolve();
      await Promise.resolve();
  ...
  it('clears Library insertion highlights after an internal Effect move consumes the drop', () => {
    ...
    transfer.setData('application/x-blue-mixer-effect', JSON.stringify({
      channelId: 'channel-1', chain: 'pre', entryId: 'pre-1',
    }));
    dispatchDragEvent(postEnd, 'drop', transfer);
  ...
  it('does not move an Effect when it is dropped back at its current boundary', () => {
    ...
    dispatchDragEvent(row, 'dragover', transfer);
    dispatchDragEvent(row, 'drop', transfer);
    expect(onPatch).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'moveChainEntryAcrossChains' }));
  });
});

...
describe('library interchange UI', () => {
  it('offers import and Export Current/All without migration or history commands', () => {
    const actions = { onImport: vi.fn(), onImportDirectory: vi.fn(), onExportCurrent: vi.fn(), onExportAll: vi.fn() };
    ...
    act(() => root.render(<>
      ...
      <LibraryImportDialog preview={{ previewToken: 'p', expiresAt: Date.now() + 1000, sources: [{
        ...
        ambiguousFolderCount: 0, folderConflicts: [],
  ...
  it('keeps import blocked until every ambiguous folder has an explicit destination', async () => {
    ...
    document.body.appendChild(container);
    const root = createRoot(container);
    act(() => root.render(<LibraryImportDialog preview={{
      previewToken: 'p', expiresAt: Date.now() + 1000, sources: [{
        sourcePath: '/tmp/udoLibrary.xml', sourceHash: 'hash', libraryType: 'udo', folderCount: 1,
        itemCount: 1, unsupportedCount: 0, exactDuplicateCount: 0, aliasConflictCount: 0,
        ambiguousFolderCount: 1,
        folderConflicts: [{
          conflictId: 'hash:0', sourceBreadcrumb: ['UDO Library', 'Shared'], candidates: [

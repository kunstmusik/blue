...
describe('RenderToDiskDialog', () => {
  ...
  it('opens from the first status broadcast and tracks the render to completion', () => {
    ...
    expect(container.querySelector('[data-testid="render-dialog-summary"]')!.textContent)
      .toBe('Generating disk CSD... (0%)');
    ...
    expect(container.querySelector('[data-testid="render-output-toggle"]')!.getAttribute('aria-expanded'))
      .toBe('true');
    expect(container.querySelector('[data-testid="render-output-text"]')!.textContent)
      .toBe('csound line one\npartial line');

    act(() => {
      statusCallback(diskStatus({ phase: 'completed', message: 'Render complete.', progress: 100 }));
    });

    expect(container.querySelector('[data-testid="render-dialog-title"]')!.textContent)
      .toBe('Render to Disk - Complete');
    expect(container.querySelector('tbody tr')!.textContent).toContain('Complete');
    expect((container.querySelector('[data-testid="render-dialog-ok"]') as HTMLButtonElement).disabled).toBe(false);
    expect((container.querySelector('[data-testid="render-dialog-cancel"]') as HTMLButtonElement).disabled).toBe(true);
    expect(container.querySelector('[data-testid="render-dialog-summary"]')!.textContent)
      .toBe('Render complete.');
  ...
  it('cancels the active render from the Cancel button and disables further cancels', () => {
    act(() => {
      statusCallback(diskStatus({ phase: 'preparing', progress: 0, message: 'Generating disk CSD...' }));
    });
    ...
    act(() => {
      statusCallback(diskStatus({ phase: 'cancelled', message: 'Render cancelled.', progress: null, outputPath: null }));
    });
    expect(container.querySelector('[data-testid="render-dialog-title"]')!.textContent)
      .toBe('Render to Disk - Cancelled');
  ...
  it('surfaces the error and marks the row failed when the render fails', () => {
    ...
    expect(container.querySelector('[data-testid="render-dialog-title"]')!.textContent)
      .toBe('Render to Disk - Failed');
    expect(container.querySelector('tbody tr')!.textContent).toContain('Failed');
    expect(container.querySelector('[data-testid="render-dialog-error"]')!.textContent)
      .toBe('Csound exited with code 1. boom');
    expect((container.querySelector('[data-testid="render-dialog-ok"]') as HTMLButtonElement).disabled).toBe(false);
  });

  it('ignores statuses for other operations and freeze statuses', () => {
    act(() => {
      statusCallback(diskStatus({ kind: 'freeze' as RenderOperationStatus['kind'], phase: 'rendering' }));
    });
    expect(useRenderToDiskStore.getState().open).toBe(false);
  ...
  it('keeps a terminal dialog settled but still surfaces late open-command failures', () => {
    ...
    expect(container.querySelector('[data-testid="render-dialog-title"]')!.textContent)
      .toBe('Render to Disk - Failed');
    expect(container.querySelector('[data-testid="render-dialog-error"]')!.textContent)
      .toBe('Open command failed: spawn ENOENT');
  });

  it('switches a settled dialog to a newly started operation and rejects old events', () => {
    act(() => {
      statusCallback(diskStatus({ phase: 'preparing', progress: 0, message: 'Generating first render...' }));
      statusCallback(diskStatus({ phase: 'completed', message: 'First render complete.', progress: 100 }));
      statusCallback(diskStatus({
        operationId: 'disk-2',
        phase: 'preparing',
    ...
    expect(container.querySelector('[data-testid="render-dialog-title"]')!.textContent)
      .toBe('Render to Disk - Running');
    expect(container.querySelector('[data-testid="render-dialog-summary"]')!.textContent)
      .toBe('Generating second render... (0%)');
    expect(container.querySelector('[data-testid="render-row-output"]')!.textContent)
      .toContain('/tmp/second.wav');

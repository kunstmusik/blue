...
beforeEach(() => {
    ...
    cutLibraryToClipboard: vi.fn(async () => ({
      ...
      value: {
        clipboard: {
          ...
          source: { kind: 'buffer' as const, clipboardId: 'udo-buffer', libraryType: 'udo' as const },
          capturedAt: 100,
        },
  ...
  rectSpy = vi.spyOn(HTMLElement.prototype, 'getBoundingClientRect').mockReturnValue({
    x: 0, y: 0, left: 0, top: 0, right: 900, bottom: 390, width: 900, height: 390,
    toJSON: () => ({}),
  } as DOMRect);
...
describe('empty UDO workspace', () => {
  it('keeps an empty drop-ready table and draggable separator visible at docked height', () => {
    ...
    const tablePane = container.querySelector('[data-split-id="udo.workspace.outer"]') as HTMLElement;
  ...
  it('matches an embedded Instrument UDO source for shared Copy and Cut', async () => {
    vi.mocked(window.blueAPI.browseLibraries).mockImplementation(async (request) => ({
      ok: true as const,
      value: request.parent.scope === 'projectOwned' ? {
        contentRevision: 1,
        parent: {
          key: null,

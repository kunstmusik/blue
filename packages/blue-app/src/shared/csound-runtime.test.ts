...
const ioReport = {
  ...
  audioInputs: [],
  audioOutputs: [{
    kind: 'audio', direction: 'output', module: 'pa_bl', deviceId: 'Built-in Output',
    displayName: 'Built-in Output', interfaceName: null, maxChannels: 2,
  }],
  midiInputs: [],
  midiOutputs: [],
  diagnostics: [],
...
describe('Csound runtime shared contract', () => {
  it('formats known module identifiers without changing their exact values', () => {
    ...
    expect(formatCsoundRuntimeModuleOption('audio', 'third-party-backend')).toBe('third-party-backend');
  });
  ...
  it('normalizes query input and ignores unknown properties', () => {
    expect(normalizeCsoundIoQueryRequest({
      audioModule: ' pa_bl\0 ',
      unknown: 'discarded',
  ...
  it('rejects shell-like utility names while preserving ordered arguments', () => {
    expect(normalizeCsoundExecutionRequest({
      kind: 'utility', operationId: 'utility-1', utilityName: 'sndinfo',
      args: ['C:\\Users\\Blue User\\file.aif'], cwd: '/tmp',
    }).args).toEqual(['C:\\Users\\Blue User\\file.aif']);
    expect(() => normalizeCsoundExecutionRequest({
      kind: 'utility', operationId: 'utility-2', utilityName: '../csound', args: [], cwd: '/tmp',
    })).toThrow('simple registered name');
    expect(() => normalizeCsoundExecutionRequest({
      kind: 'unknown', operationId: 'utility-3', utilityName: 'sndinfo', args: [], cwd: '/tmp',
    } as any)).toThrow('utility or performance');
  });
});

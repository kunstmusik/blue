...
describe('workbench-window-contract channel constants', () => {
  it('exposes stable, distinct channel names', () => {
    ...
    expect(WORKBENCH_WINDOW_UPDATE_OWNERSHIP_CHANNEL).toBe('workbench-window:update-ownership');
    expect(WORKBENCH_WINDOW_REVEAL_PANEL_CHANNEL).toBe('workbench-window:reveal-panel');
    expect(WORKBENCH_WINDOW_REQUEST_CLOSE_CHANNEL).toBe('workbench-window:request-close');
    expect(WORKBENCH_WINDOW_DOCK_GROUP_CHANNEL).toBe('workbench-window:dock-group');
...
describe('isValidDockingOrigin', () => {
  ...
  it('rejects missing or invalid originMode', () => {
    ...
    expect(isValidDockingOrigin({ ...rest, originMode: 'garbage' })).toBe(false);
  });

  it('rejects missing or invalid presentation', () => {
    const { presentation, ...rest } = validOrigin();
    expect(isValidDockingOrigin(rest)).toBe(false);
    expect(isValidDockingOrigin({ ...rest, presentation: 'hovering' })).toBe(false);
  });

  it('rejects non-string-array originPanelOrder', () => {
    expect(isValidDockingOrigin({ ...validOrigin(), originPanelOrder: 'a,b' })).toBe(false);
    expect(isValidDockingOrigin({ ...validOrigin(), originPanelOrder: [1, 2] })).toBe(false);
  });
  ...
  it('rejects non-finite optional numeric fields', () => {
    ...
    expect(isValidDockingOrigin({ ...validOrigin(), dockedSize: NaN })).toBe(false);
    expect(isValidDockingOrigin({ ...validOrigin(), slideoutSize: 'big' })).toBe(false);
  });
  ...
  it('rejects invalid close-restore placement metadata', () => {
    expect(isValidDockingOrigin({ ...validOrigin(), restoreDirection: 'diagonal' })).toBe(false);
    expect(isValidDockingOrigin({ ...validOrigin(), restoreReferenceGroupId: 42 })).toBe(false);
    expect(isValidDockingOrigin({ ...validOrigin(), auxiliaryGroupInstanceId: 42 })).toBe(false);
  });

  it('rejects an invalid auxiliarySeedGroupId', () => {
    expect(isValidDockingOrigin({ ...validOrigin(), auxiliarySeedGroupId: 'nope' })).toBe(false);
  });

  it('rejects a non-string capturedAt', () => {
    expect(isValidDockingOrigin({ ...validOrigin(), capturedAt: 123 })).toBe(false);
  });
});
...
describe('isOnScreenBounds', () => {
  it('returns true when no work areas are supplied', () => {
    expect(isOnScreenBounds({ x: -5000, y: -5000, width: 10, height: 10 }, [])).toBe(true);
  });

  it('returns true when bounds intersect a work area', () => {
    expect(isOnScreenBounds({ x: 100, y: 100, width: 400, height: 300 }, [WORK_AREA])).toBe(true);
  });

  it('returns false when bounds miss every work area', () => {
    expect(isOnScreenBounds({ x: -5000, y: -5000, width: 100, height: 100 }, [WORK_AREA])).toBe(
      false,
    );
  });
  ...
  it('checks against multiple work areas', () => {
    ...
    expect(isOnScreenBounds({ x: 2000, y: 50, width: 200, height: 200 }, [WORK_AREA, second])).toBe(
      true,
    );
  });
});
...
describe('correctOffscreenBounds', () => {
  it('enforces the minimum size floor', () => {
    const corrected = correctOffscreenBounds({ x: 100, y: 100, width: 10, height: 10 }, [
      WORK_AREA,
    ]);
  ...
  it('keeps on-screen position, only flooring size', () => {
    const corrected = correctOffscreenBounds({ x: 200, y: 200, width: 500, height: 400 }, [
      WORK_AREA,
    ]);
  ...
  it('snaps offscreen bounds onto the first available work area', () => {
    const corrected = correctOffscreenBounds({ x: -5000, y: -5000, width: 400, height: 300 }, [
      WORK_AREA,
    ]);

...
export function getBsbWidgetDisplaySize(node: BsbWidgetNodeSnapshot): {
  width: number;
  height: number;
} {
  switch (node.type) {
    ...
    case 'BSBKnob': {
      ...
      const labelFontSize =
        typeof node.properties['labelFont.size'] === 'number'
          ...
          : 12;
      const labelWidth = labelEnabled ? estimateTextContentSize(labelText, labelFontSize).width : 0;
      return {
    ...
    case 'BSBGroup': {
      ...
      return {
        width: Math.max(
          1,
          titleWidth,
          typeof node.width === 'number' ? node.width : 20,
          childrenWidth,
        ),
        height:
          labelHeight +
          Math.max(1, typeof node.height === 'number' ? node.height : 20, childrenHeight),
      };
    }
    case 'BSBLabel': {
...
function getDropdownDisplayWidthFromSnapshot(

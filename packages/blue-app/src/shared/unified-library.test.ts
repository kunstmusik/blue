...
describe('Unified Library shared contracts', () => {
  ...
  it('guards user and project item keys without accepting arbitrary shapes', () => {
    expect(isLibraryItemKey({ scope: 'user', libraryType: 'effect', nodeId: 'node-1' })).toBe(true);
    expect(
      isLibraryItemKey({
        scope: 'projectOwned',
  ...
  it('guards opaque drag descriptors, exact targets, and revision-bound clipboard entries', () => {
    ...
    expect(
      isLibraryDragDescriptor({
        dragSessionId: 'drag-project-effect',
        libraryType: 'effect',
        sourceScope: 'projectOwned',
      }),
    ).toBe(true);
    expect(
      isLibraryDragDescriptor({
        dragSessionId: 'drag-invalid-scope',
        libraryType: 'effect',
        sourceScope: 'project',
      }),
    ).toBe(false);
    ...
    expect(
      isLibraryExactTransferTarget({
        kind: 'projectSoundObjectLibrary',
        projectSessionId: 4,
        projectRevision: 7,
      }),
    ).toBe(true);
    ...
    expect(
      isLibraryExactTransferTarget({
        ...
        projectSessionId: 4,
        projectRevision: 7,
        track: { rootGroupId: 'tracks', trackId: 'lead' },
        insertIndex: 0,
      }),
    ).toBe(true);
    expect(
    ...
    expect(
      isLibraryInteractionClipboard({
        ...
        source: {
          kind: 'library',
          key: { scope: 'user', libraryType: 'udo', nodeId: 'u-1' },
          revision: 3,
        },
        capturedAt: 100,

...
describe('Library editor document contracts', () => {
  it('accepts typed supported documents and rejects raw-XML supported documents', () => {
    expect(isLibraryEditorDocument({
      kind: 'udo',
      snapshot: {
        name: 'SoftClip', style: 'CLASSIC', outTypes: 'a', inTypes: 'a',
        inputArguments: '', code: 'xout ain', comments: '',
      },
    })).toBe(true);
  ...
  it('guards discriminated native patches', () => {
    expect(isLibraryEditorDocumentPatch({
      kind: 'effect',
      patch: { name: 'Reverb' },
    })).toBe(true);

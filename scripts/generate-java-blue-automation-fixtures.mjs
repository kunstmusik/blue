...
function buildJavaBlue(javaBlueRoot) {
  run(
    "mvn",
    ["-q", "-pl", "blue-core,blue-ui-core", "-am", "-DskipTests", "package"],
    { cwd: javaBlueRoot, label: "mvn package blue-core,blue-ui-core" },
  );
}
...
function compileGenerator(classpath) {
  ...
  run("javac", ["--release", "25", "-encoding", "UTF-8", "-cp", classpath, "-d", outDir, ...sources], {
...
function main() {
  ...
  if (args.check) {
    ...
    try {
      ...
      console.log(`[fixtures] regenerated corpus written to ${path.relative(REPO_ROOT, CORPUS_DIR)}`);
    } finally {

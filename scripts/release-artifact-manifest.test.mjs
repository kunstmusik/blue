...
try {
  ...
  assert(generated.status === 0, `package manifest generation failed:\n${generated.stderr}`);

  const valid = validate();
  assert(valid.status === 0, `valid package manifest was rejected:\n${valid.stderr}`);
  ...
  assert(
    manifest.targets.every((target) => target.path === target.path.split('/').at(-1)),
  ...
  writeFileSync(macPackagePath, packageFiles[`blue-macos-arm64-${appVersion}.dmg`]);
  ...
  assert(mismatchedArchitecture.status === 1, 'incorrect target architecture must fail validation');
  assert(
  ...
  assert(duplicateFormat.status === 1, 'duplicate target formats must fail validation');
  assert(
  ...
  const debPackagePath = join(fixtureDir, `blue-linux-x64-${appVersion}.deb`);
  ...
  writeFileSync(debPackagePath, packageFiles[`blue-linux-x64-${appVersion}.deb`]);
  ...
  process.stderr.write('PASS release artifact manifest generation and validation\n');
} finally {

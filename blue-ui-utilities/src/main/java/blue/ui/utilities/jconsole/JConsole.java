 ...
 *
 * Original Author: Android Updated by Steven Yi for blue
 *
 * @author steven yi
 */
public class JConsole extends JTextArea implements KeyListener {
        ...
        if ((e.getModifiers() & shortcutKey) == shortcutKey) {
            ...
            if (!e.isShiftDown() && !e.isAltDown()) {
                if (running) {
                    ...
                    pythonThread = new Thread(new JConsoleCommandRunner(command));
    ...
    private class JConsoleCommandRunner implements Runnable {
        ...
        public void run() {
            running = true;
            try {
                delegate.processCommands(commands, in, out, err);
            } catch (Exception e) {
                // catch any exceptions and print to the log 
                e.printStackTrace();
            }

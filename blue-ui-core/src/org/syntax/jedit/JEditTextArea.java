 ...
 * @author Slava Pestov
 * 
 * Modified by Steven Yi to work with blue
 */
...
public class JEditTextArea extends JComponent {
    ...
    private int getMaxTextWidth() {
        ...
        for (int i = 0; i < lineText.length(); i++) {
            if (lineText.charAt(i) == '\t') {
                charCount += (tabWidth - (charCount % tabWidth));
            } else {
                charCount++;
            }
        }

        int lineMax = charCount * painter.getFontMetrics().charWidth('w');
    ...
    public void setFirstLine(int firstLine) {
        ...
        // Taken from JEdit CVS
        int max = getLineCount() - visibleLines;
        // + (lastLinePartial ? 1 : 0);
    ...
    class MouseWheelHandler implements MouseWheelListener {
        public void mouseWheelMoved(MouseWheelEvent e) {
            /*******************************************************************
             * move caret depending on pressed control-keys: - Alt: move cursor,
             * do not select - Alt+(shift or control): move cursor, select -
             * shift: scroll page - control: scroll single line - <else>: scroll
             * 3 lines
             ******************************************************************/
            if (e.isAltDown()) {

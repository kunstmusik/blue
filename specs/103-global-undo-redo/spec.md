...
A composer undoing or redoing while playing hears supported live changes and can distinguish a restored project from a performance that still needs restarting.
...
2. **Given** a reversal requiring a new compiled performance, **When** the project is restored, **Then** the UI indicates that restart is required, playback is not automatically interrupted, and the next user-requested restart uses the restored project.
...
- **FR-008**: A completed drag or compound command MUST be one undo step. Typing MUST group within the same field while gaps remain below 500 ms, preserving entire input-method compositions. Selection movement, a different editor/action, save, undo, or redo MUST end grouping; interleaved actions MUST remain separately ordered.
...
- **FR-013**: History retention MUST be bounded by both action count and retained data size. Eviction MUST remove complete oldest actions only, communicate the available limit, and never misidentify an evicted checkpoint as clean. An action too large to retain MUST be identified before commitment and require explicit user agreement to a history reset or allow cancellation.
...
- **FR-015**: Supported live reversals MUST update every applicable active performance, including normal playback and Blue Live, in action order. The UI MUST distinguish pending, applied, restart-required, and failed runtime outcomes and MUST not report pending work as applied.
- **FR-016**: A change requiring compilation MUST visibly mark the active performance as requiring restart without automatically interrupting playback. The next user-requested start/restart MUST use the current canonical project, including restored code, score, effects, and instruments.
- **FR-017**: Runtime failure MUST NOT reverse a successfully committed document action or corrupt history. The UI MUST offer an applicable retry or restart recovery action; obsolete results MUST not affect a different project or performance.
...
- **SC-004**: In that suite with a responsive engine, 95% of supported single-parameter live reversals are acknowledged within 250 ms; all injected failures and restart-required changes produce a visible accurate status within one second of their determination.

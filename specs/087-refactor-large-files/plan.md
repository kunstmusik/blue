...
- **Java and project compatibility**: PASS — XML load dispatch, save section ordering, `pluginDataXml` unknown-data preservation, and raw-XML migration ordering move verbatim; `UpgradeManager.performUpgrades` still runs before deserialization. CSD policy moves verbatim; the sync/async duplication is relocated as-is (unification is explicitly out of scope). The current `blue-data-csd-parity.test.ts` and `blue-data-csd-disk.test.ts` suites use self-contained projects; the developer-local Demo2026 and rhythmic reference cases were removed on 2026-09-24. The in-repo `migration/fixtures/track-layer/*.blue.xml` integration test remains. No intentional divergence.

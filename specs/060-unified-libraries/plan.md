...
**Primary Dependencies**: built-in `node:sqlite` (`DatabaseSync` and `backup`) and `node:worker_threads` in Electron main; existing `@rgrove/parse-xml` and `@blue/data` models/codecs; Electron `app`/`dialog`/IPC; Dockview 5.2.0; Zustand 5.x; Radix menus; existing type-specific Instrument/UDO/Effect/SoundObject editors
...
- Keep one main-owned editor session per stable item identity and present the selected session through exactly one transient Dockview panel titled `Library Item`; retain dirty/pinned drafts outside the visible slot, reopen them by identity, and discard restored session-bound tabs from saved layouts. Retain the current address header and replace the raw XML body with native type-specific editors.
...
5. Register `SoundObjectLibraryTopComponent` as a separate right-side auxiliary panel backed by canonical Project Shared SoundObjects. It reuses the Library Item editor routing, the single cross-owner typed clipboard/drag session, and guarded project deletion services.
...
8. Treat every browse response as one page, not a complete collection. Initial presentation loads only root metadata; expansion drains every opaque cursor page with the first page's content revision, stable ordering, node-ID deduplication, repeated-cursor rejection, surfaced failures, and generation guards so stale refresh/search/expansion responses cannot replace newer state. Apply the same complete-page rule to Project SoundObjects and project UDO lookup.
9. Complete desktop tree organization semantics: folders and items support contextual Cut/Copy, Paste on an item resolves to its parent, compatible folder-to-folder drag emits a guarded revision-bound move, Move Up/Down emits reorder mutations, roots cannot rename/delete/move, and non-empty deletion uses the affected-count confirmation service. Search rows display type, ownership, and breadcrumb context.
10. Route bidirectional project/user transfer through one typed clipboard and opaque drag service for Orchestra Instruments, project UDOs, mixer Effects, and Project Shared SoundObjects. Copy creates an independent destination. Cut captures a detached deep typed snapshot, removes the validated source immediately, and leaves that snapshot reusable for independent Paste operations.
11. Offer both XML-file and Java-configuration-directory manual import. Preview duplicate-folder candidates by stable node identity and full breadcrumb, keep apply disabled until every ambiguity is selected, bind selections to source hashes, and validate the chosen identities again inside the per-source transaction.
12. Hold one interchange lease across Export Current and Export All. Fully build and reparse every requested Java-compatible output before presenting a compatibility/overwrite summary; Cancel writes nothing, approval atomically promotes all outputs, and promotion failure restores every prior destination.
13. Address a project UDO by both its owning list and stable session identity. Omit the owner for the top-level project list; carry the Instrument assignment ID for a local UDO list. Reuse this exact locator/target in embedded Instrument UDO editors so drag, Copy, Cut, and Paste work bidirectionally with the user UDO Library without a second buffer or fallback to the top-level list.
14. Render folder/root disclosure from node kind rather than child count, using a large high-contrast arrow so an empty folder remains visibly expandable and never resembles an item.
15. Treat the flexible remainder below the last UDO or Effect entry as the exact end-insertion target, reusing the existing guarded target and Paste context while expanding only its hit/highlight geometry. Give mixer strips a readable vertical floor and scroll the mixer body below that floor so fixed-size slider content cannot escape a collapsed level region and overlap adjacent labels.
...
- Add single-visible-tab switching, retained dirty/pinned session, address-header retention, and supported-type native-editor tests; assert the raw XML textarea is absent for supported items.
...
- Replace panel-level actions with one icon-only vertical-ellipsis popup labeled `Library actions`; include Import XML, Import Java Configuration Directory, Export Current, and Export All. Do not expose Import History, Migration Report, or routine migration status. Only repository recovery may replace normal panel content.
...
- Add Radix context menus reachable by right-click and `Shift+F10`/Context Menu key. Compute commands by node/scope capability and expose applicable Duplicate, Cut, Copy, Paste, Delete, and folder creation with destructive confirmation/affected counts; do not add type-specific project-to-user commands.
- Add one transient typed clipboard shared by user and project panels. Copy resolves an expected revision and creates a deep destination copy. Cut captures a main-owned detached snapshot before guarded immediate removal; Paste always copies that snapshot and never performs or retries source cleanup. Stale references, dirty editors, declined shared-object consequences, or failed removal preserve both the source and prior clipboard.
...
- Keep one visible selection editor slot. Selection always switches that slot to the requested existing or new session; dirty/pinned sessions remain retained by identity while hidden and return when reselected. Do not restore transient session-bound tabs from a saved layout. Unsupported or missing items use a deliberate read-only compatibility/missing state, not a generic XML editor.
...
- Cross-owner drag copies; internal project drag may move where the native model supports it, including mixer Effect chains. Shared SoundObject drop/paste pauses only for the explicit `Copy Instance` versus `Copy Independent` choice; dependency/conflict disclosures remain blocking and revision-bound.
...
- Confirm a valid drop needs no extra confirmation, every invalid/stale drop produces zero mutation, and 100 selection changes never create a second Library Item tab or lose a dirty/pinned draft.

This slice intentionally did not redesign persistence, import/export compatibility, recovery, native project copy semantics, or the `.blue` model. The corrective renderer/contract/test work and its later parity refinements are complete in `tasks.md`.

...
The renderer puts only `dragSessionId`, `libraryType`, and `sourceScope` in `DataTransfer`; payload XML remains main-owned. Orchestra, top-level, Arrangement-instrument, and Track-owned UDO tables, Mixer, Score, and user-library folders resolve exact targets from their current geometry or keyboard Paste context. A `projectUdo` target or source locator carries either an optional Arrangement assignment ID or an optional Track identity (`rootGroupId` plus `trackId`); omission addresses the top-level list, and providing both owners is invalid. Main revalidates source revision, support status, project session, owning Instrument/list, locator, target revision, dependencies, and destination immediately before apply. A stale, missing, incompatible, or ambiguous source/target returns no destination change. The active typed clipboard reference is main-owned and broadcast to every renderer window. A separate validated `bsbClipboard` snapshot field and setter channel carry only BSB widget payloads between main and detached instrument-editor renderers; it never satisfies a Library Paste. Cut is a separate main-owned transaction boundary: capture a detached typed snapshot, perform guarded immediate source deletion, then return only an opaque buffer identity; Paste resolves that buffer without any source cleanup.

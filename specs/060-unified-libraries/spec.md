...
2. **Given** a user-library UDO and an active project, **When** the user drops or pastes it at a top-level project UDO or Instrument-local UDO table insertion position, **Then** that exact UDO list receives an independent copy at the indicated position.
...
- **FR-020b**: One application clipboard MUST support every permitted user and project Instrument, UDO, Effect, SoundObject, and user-folder Cut/Copy operation without exposing raw XML to the renderer. Copy stores a typed revision-bound source reference. Cut MUST validate and capture a complete detached deep snapshot before removing the source immediately; any required shared-project consequence confirmation occurs before removal. If capture, confirmation, or removal fails, the source and prior clipboard MUST remain unchanged. Paste MUST create an independent destination copy with a new destination-appropriate identity, MUST NOT perform source cleanup, and MUST leave the detached Cut buffer available for repeated Paste operations.
- **FR-020c**: Project and user panels MUST share the same Copy, Cut, Paste, and drag vocabulary for cross-owner transfer. Project items, including Instrument-local UDOs, MUST be draggable to matching user-library folders; user items MUST be draggable to compatible project targets, including the exact Instrument-local UDO table; project Effects MUST be movable between mixer chains/channels by native drag; and no type-specific `Copy to User Library` command or second clipboard may appear. Copy and Cut MUST be enabled for a selected Instrument-local UDO and use the same typed buffer as the top-level project UDO workspace and user UDO Library. Native internal drag may move an existing project item, but Cut/Paste MUST always use detached copy-then-remove-at-Cut semantics.
...
- **FR-024**: The project UDO workspace MUST always show its table and splitter even when the list is empty. Dropping or pasting a user-library UDO into an empty or populated top-level project or Instrument-local UDO table MUST add an independent entry to that exact addressed list and preserve every same-name UDO rather than replacing or reusing one implicitly. All unused table space below the final row MUST act as the exact end-insertion target and show full-area compatible-drag feedback. UDOs dragged, copied, or cut from either project UDO surface MUST resolve through the same stable project transfer path when pasted into the user UDO Library.

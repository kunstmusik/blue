...
**Input**: User description: "Review .tmp-research/MIXER_PANNING.md and independently review the codebase. Support mixed mono and stereo audio clips on a track, automatically route mono clips to both stereo channels, and add a mixer-level panning setting to preserve backwards compatibility with legacy projects while supporting industry-aligned Blue/Csound panning."
...
A composer can open an older project and get its previous audio behavior, then explicitly enable the new panning behavior from Mixer Settings when they are ready to adopt it.
...
**Independent Test**: Open a representative pre-feature project with no mixer panning setting, render it before and after a save/reopen cycle, confirm that panning is disabled and its prior routing is preserved, then enable panning in Mixer Settings and confirm that the new behavior is applied only after the explicit change.

**Acceptance Scenarios**:

1. **Given** a project created before this feature and containing no mixer panning setting, **When** it is opened, **Then** panning is disabled and the project retains legacy audio routing.
2. **Given** a newly created project, **When** Mixer Settings are opened, **Then** panning is enabled by default and the setting is visible as an editable mixer option.
3. **Given** a legacy project with panning disabled, **When** the composer enables panning and confirms the change, **Then** the project becomes dirty, the setting is stored with the mixer, and subsequent rendering uses the new mono/stereo behavior.
...
- A project with no mixer element, a mixer element without the panning setting, or an invalid panning value loads with panning disabled and keeps legacy behavior; a legacy score-level value is migrated when present.
...
- The first release covers mono and stereo audio clips coexisting on one track, a mixer-level compatibility setting, channel-level Pan/Balance controls, persistence, automation, and runtime reconciliation.
...
- **FR-001**: The mixer MUST expose an `Enable Panning` setting owned by the mixer and persist it as a mixer-level boolean property named `panningEnabled`.
- **FR-002**: A newly created mixer MUST default `panningEnabled` to true. A mixer loaded without the property, including a Java-authored or pre-feature project, MUST default it to false. Legacy `score`-level `panningEnabled` values MUST migrate into the mixer when no mixer-level value is present.
- **FR-003**: When mixer `panningEnabled` is false, rendered audio MUST retain the project's prior channel-index routing and MUST NOT apply channel Pan/Balance processing or mono center-upmixing introduced by this feature.
...
- **FR-009**: The mixer UI MUST label or otherwise distinguish Mono Pan from Stereo Balance, MUST make the control unavailable or clearly inactive when mixer panning is disabled, and MUST expose the mixer-level setting in Mixer Settings.
...
- **FR-011**: Changing the mixer panning setting or a channel position MUST use the canonical project history path with a semantic history label, and commit, undo, redo, dirty-state publication, and running-engine reconciliation MUST restore the corresponding canonical and audible state.
...
- **FR-015**: Loading a supported Java Blue project without mixer `panningEnabled` MUST remain successful and must preserve its prior audio behavior. A legacy score-level panning property MUST be migrated without corrupting unrelated project data. The TypeScript-only panning property MUST be additive.
- **FR-016**: Deterministic verification MUST cover mono center/endpoint behavior, stereo preservation, mixed-track behavior, legacy disabled behavior, mixer-setting persistence, automation, undo/redo, save/load, runtime reconciliation, missing metadata, mono output, and unsupported multichannel inputs.
...
- **Compatibility Requirements**: A mixer without `panningEnabled` MUST load with panning disabled and retain its legacy channel routing through save/reopen. Legacy score-level panning values MUST be migrated to the mixer. Existing Java-compatible score and mixer data, unknown project data, send routing, and non-panning renders MUST remain readable and intact. New mixers may persist the explicit enabled default.
- **Intentional Divergences**: New mixers opt into panning by default, while legacy projects remain opted out. When enabled, mono clips are center-upmixed and eligible channel controls become audible; this is an intentional TypeScript/Blue Electron extension behind an explicit mixer setting. True-stereo, M/S, and multichannel speaker-layout strategies are deferred rather than inferred.
- **State Ownership**: The canonical owner of `panningEnabled` is the active mixer in `BlueData`, persisted in the mixer's `.blue` XML element. Canonical channel Pan/Balance values are also owned by the mixer channel model and persisted with mixer project data. Renderer snapshots and generated Csound text are derived representations. The Electron main process owns canonical publication and runtime reconciliation; renderer settings and controls submit typed project-history edits.
- **Undo/Redo Impact**: Toggling mixer `panningEnabled` and editing a channel position are durable project mutations. Each MUST have a semantic history entry, restore the prior canonical value and audio behavior on undo, reapply it on redo, preserve stable project identities, update dirty state correctly, and reconcile the running engine when applicable.

### Key Entities *(include if feature involves data)*

- **Mixer Panning Configuration**: A mixer-owned compatibility setting that determines whether the new panning and mono center-upmix behavior is active; it has an explicit enabled/disabled state and a legacy default for absent data. Legacy score-level values are accepted only as a load-time migration source.
...
- **SC-003**: 100% of tested pre-feature projects with no mixer `panningEnabled` property produce the same legacy routing before and after save/reopen, and none enable panning without an explicit user change.
- **SC-004**: 100% of new-mixer creation and mixer-settings tests show panning enabled by default and persist the explicit setting through save/load.
...
- **SC-009**: A composer can identify whether the active control is Pan or Balance and enable or disable mixer panning from Mixer Settings without consulting external documentation.

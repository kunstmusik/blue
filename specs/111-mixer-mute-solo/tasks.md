...
- [X] T026 [P] [US2] Add XML/default/copy regression cases for new-project Audio, missing score Event fallback, missing mode attribute Event fallback, explicit Audio/Event values, invalid raw values, untouched legacy omission, explicit replacement of an invalid raw value, unknown-data preservation, and master-solo compatibility in `packages/blue-data/src/score/score-mute-solo-mode.test.ts`, `packages/blue-data/src/blue-data-frozen-roundtrip.test.ts`, and `packages/blue-data/src/blue-data/xml-policy.ts`.
...
- [X] T030 [US2] Implement Score-owned `trackLayerMuteSoloMode` attribute persistence and provenance in `packages/blue-data/src/score/score.ts` and `packages/blue-data/src/blue-data/xml-policy.ts`: honor the data-model rule `trackLayerMuteSoloMode: audio/event; New Audio, loaded absence/invalid Event. Preserve unsupported raw value until explicit replacement.`, retain presence/raw text across copy/save/load, default an absent score or mode attribute to Event, and expose an unsupported-value diagnostic. No migration or fallback reads the unreleased ProjectProperties location or score child.
...
Task: "T026 XML compatibility in packages/blue-data/src/score/score-mute-solo-mode.test.ts"
...
- [X] T062 CRITICAL [US2] Make `BlueData` XML loading treat an absent `score` block as loaded legacy Event mode with `trackLayerMuteSoloModePresent=false`, rather than retaining the fresh-project Audio default; preserve the untouched omission on save and add an enabled-mixer root fixture covering load/save/reload in `packages/blue-data/src/blue-data/xml-policy.ts` and the compatibility tests per FR-007, US2/AC2, T026, and T030 (contradicts).
- [X] T063 HIGH [US2] [US4] Carry `trackLayerMuteSoloModePresent` and retained raw mode text through `ScoreSnapshot`, score patch application, and scalar history records; restore omitted and invalid source metadata exactly through commit→undo→redo and save/reload, and clear invalid raw text on an explicit Event or Audio replacement even when the parsed mode is already Event in `packages/blue-app/src/shared/project-editor/contract.ts`, `snapshot-score.ts`, `patch-score.ts`, and `main/project-history-memento.ts` with focused tests per FR-007, FR-014, US4/AC1-2, T031, T048, and T050 (partial).
...
**Purpose**: Close residual provenance and acceptance-evidence gaps found after the Phase 8 convergence tasks were implemented.

- [X] T070 MEDIUM [US2] Correct legacy mixer-state provenance so an explicitly persisted `trackLayerMuteSoloMode` (especially `audio`) with active non-master channel flags is not mislabeled as a legacy project after save/reload, while a document with the mode omitted still receives the FR-015 notice; keep live edits, master-solo exclusion, clearing behavior, and disposable XML-to-snapshot-to-Project-Properties presentation covered in `packages/blue-data/src/blue-data/xml-policy.ts`, `packages/blue-data/src/blue-data/xml-policy-mute-solo.test.ts`, `packages/blue-app/src/shared/project-editor/snapshot-mixer-orchestra.ts`, and the related contract/UI tests.
- [X] T071 MEDIUM [US3] Complete app-level export immutability evidence not covered by the current T069 tests: exercise successful, failed, and cancelled `saveGeneratedCsdToDisk`/`executeRenderToDisk` flows through the owning session/history harness, including synchronous and asynchronous generation, and assert unchanged project XML, dirty state, history cursor/entry count, stable references, runtime state, and absence of publication/history mutations in `packages/blue-app/src/main/csd-export.test.ts`, `packages/blue-app/src/main/csd-generation.test.ts`, `packages/blue-app/src/main/render-to-disk.test.ts`, and the relevant project-history test support.
- [X] T072 MEDIUM [US3] Make the real-engine optimized/unoptimized acceptance comparison satisfy the planned float-audio contract rather than relying on the current default WAV format: request and assert matching 32-bit float WAV output, compare normalized samples at the `-120 dBFS` peak-residual threshold, and record the run and fallback behavior in `packages/blue-app/src/main/mixer-mute-solo.integration.test.ts`, `packages/blue-data/src/test-support/csd-render-fixtures.ts`, and `specs/111-mixer-mute-solo/quickstart.md` per SC-004 and T045.

---

## Phase 10: Convergence

**Purpose**: Close the remaining export-state evidence gap after the Phase 9 tasks were implemented.

- [X] T073 MEDIUM [US3] Strengthen the T071 export immutability regression with a non-empty/dirty `ProjectHistory` state and an attached runtime reconciliation/performance harness; exercise successful, failed, and cancelled `saveGeneratedCsdToDisk` and `executeRenderToDisk` flows through both synchronous and asynchronous generation, and assert unchanged history cursor/entries, dirty state, canonical object identities, runtime outcomes, and publication events in `packages/blue-app/src/main/csd-export-immutability.test.ts` and the relevant project-history/runtime test support per SC-005 and T071.
...
**Purpose**: Close gaps found by a full present-state re-review of T001–T073, including completed tasks and earlier convergence phases. Existing task text and completion marks are retained; they are not evidence that the gaps below are resolved.

**Review scope (2026-09-16)**: 16 functional requirements, 5 success criteria, 19 acceptance scenarios, 11 edge cases, 10 plan obligations, all 5 constitution principles, and all 73 existing tasks. Focused validation passed 292 tests across 31 files: data 107, app 108, engine-client 47, real-engine integration 13, and browser 17. Browser tests passed after retrying Chrome outside the sandbox. These results do not establish the missing audio, hardware-latency, or Windows acceptance evidence below. No application code, spec, plan, or existing task was changed by this review.

- [X] T074 CRITICAL [US1] [US4] Fence in-flight mixer gate publications to the originating document and performance generation in `packages/blue-app/src/main/mixer-mute-solo-runtime.ts` and `packages/blue-app/src/main/main.ts`; invalidate publications on stop/replacement and check their validity after every awaited stage, retry, commit, and readback, using transport bound to that generation so old work cannot reach a replacement engine. The current publisher continues its remaining batch and commit after `reset`/`setBindings`, while main's I/O closures resolve the current session. Add delayed-stage and delayed-retry regressions through production reconciliation for timeline and BlueLive, asserting no writes to the replacement session and no stale applied outcome per Constitution III (required runtime reconciliation and explicit failure contracts), FR-011, FR-016, T014, T021, T022, and T047 (contradicts).
- [X] T075 CRITICAL [US1] [US4] Preserve compiled gate ownership across track/channel topology edits in `packages/blue-data/src/mixer/mute-solo-policy.ts`, `packages/blue-data/src/blue-data/csd-policy.ts`, `packages/blue-app/src/main/main.ts`, and `packages/blue-app/src/main/project-runtime-reconciliation.ts`; capture stable canonical channel/entry identities before cloning and either map current intent into the compiled order or require restart when ownership changes. The current signature encodes ordinals/kinds/targets but no source identity: reordering two direct-to-master Tracks and then renaming one changes the canonical vector from `[0,1,1]` to `[1,0,1]` with the same signature, and restart ownership for score/mixer edits does not fence `mixer-gates`. Add real patch/history regressions for reorder→rename→M/S and undo/redo that prove the running engine never gates the wrong original channel per Constitution III (identity-preserving runtime reconciliation), FR-013, US4/AC1, plan: pre-clone identity capture, T005, T018, T022, and T051 (contradicts).
- [X] T076 HIGH [US1] [US4] Reconcile uncertain commit writes and failed applied-token reads before reusing either gate bank in `packages/blue-app/src/main/mixer-mute-solo-runtime.ts`; retain the attempted token before awaiting its acknowledgment, distinguish definite rejection from uncertain transport completion, and recover from engine readback or require restart when safety cannot be established. Reproduction: the engine accepts token 1 but the response throws, leaving publisher token 0; the next edit stages into already-audible bank 1 and republishes token 1. Cover lost commit responses, readback exceptions, subsequent edits, and undo/redo without partial audible vectors or false applied status in the publisher/reconciliation tests per FR-016, plan: atomic two-bank publication, T014, T021, and T047 (partial).
- [X] T077 HIGH [US3] Enforce the planned conservative project certification in `packages/blue-data/src/blue-data/csd-policy.ts` and strengthen `packages/blue-data/src/blue-data/csd-disk-pruning.test.ts`; inspect enabled Track-owned instruments and their authored global/always-on/UDO code as well as arrangement instruments, and retain events when mixed content, processors, or unknown executable extensions elsewhere prevent establishing independence. A muted AudioClip track currently loses its event even when another Track's authored global orchestra is emitted. Existing mixed-content/processor/unknown-content tests merely find a shared `test.wav` string that the other audible track already supplies. Use distinct per-track event identities/counts and sync/async assertions proving every required fallback retains the candidate's event, while certified fixtures still prune, per FR-012, US3/AC3, plan: disk certification, T038, T041, and T068 (contradicts).
- [X] T078 HIGH [US1] [US2] [US4] Render persistent playback-application status in the mixer/header workflow using `packages/blue-app/src/renderer/stores/project-store.ts` and the owning mixer/score components; `runtimeOutcomeStatusText` and `runtimeOutcomes` currently have no production visual consumer, and only failed outcomes emit a transient toast. Show pending, failed, and restart-required state without implying that saved channel flags are applied, distinguish timeline and BlueLive when outcomes differ, and retain an actionable recovery explanation until reconciliation or restart clears it. Add rendered browser assertions for mode/bypass restart feedback and mixed performance success/failure per FR-011, FR-016, US4/AC3, T009, T024, T036, and T052 (partial).
- [X] T079 HIGH [US2] Include a legacy stored master mute in compatibility-notice provenance and presentation in `packages/blue-data/src/blue-data/xml-policy.ts` and `packages/blue-app/src/shared/project-editor/snapshot-mixer-orchestra.ts`; both checks currently inspect only source/subchannels, so a legacy master-mute-only project becomes silent without the required notice. Continue ignoring master solo by itself and preserve modern explicit-mode provenance. Add load→snapshot→UI and save/reload cases for master mute, master solo, their combination, bypass/re-enable, and clearing the active mute per FR-005, FR-015, T031, T034, T067, and T070 (partial).
- [X] T080 HIGH [US1] [US4] Complete live generated-audio and history acceptance in `packages/blue-app/src/main/mixer-mute-solo.integration.test.ts` and `packages/blue-app/src/main/global-history-engine.integration.test.ts`; current gate tests use handwritten silent orchestras that echo commit tokens and do not establish behavior of generated BlueMixer audio. Run generated timeline and BlueLive performances with distinguishable sources, pre/post sends, soloed/shared returns, multiple solos, mute-wins, master restoration, and stateful tails; inspect captured samples for isolation and atomic publication above 256 routes, and verify transport/event continuity. Exercise channel/header M/S, mode, and mixer-enable commit→undo→redo with canonical flags, identities/associations, dirty/publication state, independent simultaneous outcomes, and one performance failing. Record actual coverage and results in `specs/111-mixer-mute-solo/quickstart.md` per SC-001, SC-005, US1/AC1-6, US4/AC1-3, T013, T016, T025, T046, T049, T054, and T058 (partial).
- [X] T081 HIGH [US3] Complete deterministic disk equivalence and scheduling evidence in `packages/blue-app/src/main/mixer-mute-solo.integration.test.ts`, `packages/blue-data/src/blue-data/csd-disk-pruning.test.ts`, and the scheduling/determinism suites: extend the single direct-output float comparison to pre/post sends, soloed returns, muted master/all-pruned output, nonzero render start/end, nonconstant tempo, surviving fades/loops, global duration, extra render time, and conservative fallback fixtures. The current “nonzero window” test sets only render end, its tempo test uses default 60 BPM, and its fade/loop test prunes the only configured fade/loop clip. Compare genuinely equivalent optimized/unoptimized fixtures at identical sample counts and ≤−120 dBFS peak residual, assert exact event identities/counts, and cover sync/async output. Record results in `specs/111-mixer-mute-solo/quickstart.md` per SC-004, FR-012, T002, T039, T042, T043, T045, T069, and T072 (partial).
- [ ] T082 HIGH [US1] Complete the still-outstanding physical acceptance from T065 after the runtime fixes: measure 100 warmed-up UI-to-audio transitions on timeline and BlueLive with the documented 32-source/64-send reference and a >256-route case, using a real output/loopback device; confirm each transition settles within 100 ms without transport jumps, event retriggering, or intermediate route combinations. Record machine/device/version/settings and results in `specs/111-mixer-mute-solo/quickstart.md`; retain the explicit blocked status if hardware is unavailable rather than treating protocol timing as completion. This carries forward the existing unchecked T065, whose `US1/AC7` reference should be read as the current US1/AC6, per SC-002, US1/AC6, T016, T025, T059, and T065 (partial).
- [X] T083 MEDIUM [US2] Complete actual browser interaction coverage in `packages/blue-app/src/renderer/browser/track-header-mute-solo.browser.test.tsx` and the mixer browser suite: use trusted browser keyboard actions to activate header M/S and assert the resulting patch/state, rather than dispatching a synthetic keydown and checking only focus; mount the mode selector and cover explicit Event mode, Audio↔Event switching with independent flags, bypass/re-enable, and synchronized header/strip views. Keep missing-association and renamed-association coverage per FR-006 through FR-009, US2/AC1-6, T029, T057, and T066 (partial).
- [ ] T084 MEDIUM Run and record the feature's supported native Windows validation in `specs/111-mixer-mute-solo/quickstart.md`, including portable data/contract/history tests, app/engine-client checks, browser controls, and available real-engine float-render/gate tests with native paths; add the specifically planned synthetic Windows embedded-path/error coverage where absent. The current recorded runs identify only darwin arm64 and still call for native Windows validation. Report unavailable engine/device prerequisites as outstanding rather than passed per plan: host-path portability and target-platform validation, T055, and T060 (missing).
...
**Purpose**: Close residual implementation and acceptance-evidence gaps after the Phase 11 implementation pass. This assessment reviewed T001–T084 against the current spec, plan, and constitution; previous completion marks remain unchanged.

**Validation**: Focused data tests passed 83/83, app tests 102/102, real-engine integration 16/16, and browser tests 14/14 (215 tests across 21 files). A targeted strict TypeScript check of the new runtime-status component reports TS2307 and two TS2366 diagnostics. Passing transpilation/browser tests do not resolve these type errors. Hardware latency and native Windows acceptance remain outstanding.

- [X] T085 CRITICAL Correct the shared runtime-outcome type import in `packages/blue-app/src/renderer/components/workbench/RuntimeOutcomeStatus.tsx`: `../../shared/project-history` resolves to nonexistent `src/renderer/shared/project-history`; the canonical contract is `../../../shared/project-history`. Verify the component under strict TypeScript, including the exhaustive status helpers, and include renderer type checking in the recorded feature validation rather than relying on Vite/Vitest transpilation. The current targeted check reports TS2307 at the import and TS2366 in both status helpers per Constitution: TypeScript and Import Discipline, Constitution V, plan: strict TypeScript, T060, and T078 (contradicts).
- [X] T086 HIGH Preserve unresolved restart/failure state across later successful edits in `packages/blue-app/src/main/project-runtime-reconciliation.ts`, `packages/blue-app/src/renderer/stores/project-store.ts`, and the runtime-status presentation. Reproduction: committing `trackLayerMuteSoloMode: event` returns restart-required for `projectProperties`, then a mixer mute on the same generation returns applied and replaces the warning even though its compiled event mode is unchanged; the UI now says saved values are active. Track outstanding reconciliation obligations per performance until actually resolved or restarted, while allowing safe unrelated live operations. Add mode/event/mixer-enable→M/S and failed-operation→unrelated-success regressions, including independent timeline/BlueLive outcomes and rendered persistent feedback, per FR-011, FR-016, US4/AC3, T036, T052, and T078 (contradicts).
- [X] T087 HIGH Complete stable compiled identity binding for channels and entries without Track associations in `packages/blue-data/src/mixer/mute-solo-policy.ts`, `packages/blue-data/src/blue-data/csd-policy.ts`, and the host catalog/reconciliation wiring. The associated-track reorder case now changes the signature, but `getChannelIdentity` still uses channel display names for unassociated channels; replacing a subchannel with a different object named `Return` produces the same `channel:sub:Return` identity and signature. Capture canonical editor channel/entry identities before render cloning, or invalidate gate bindings reliably for every structural replacement, so same-name replacements cannot be accepted against an obsolete performance. Cover actual structural patches and undo/redo, not only a manually substituted signature, per plan: pre-clone canonical identity capture, FR-013, T005, T018, T051, and T075 (partial).
- [X] T088 HIGH Finish the remaining generated live-audio/history acceptance from T080 in `packages/blue-app/src/main/mixer-mute-solo.integration.test.ts` and `packages/blue-app/src/main/global-history-engine.integration.test.ts`. The added realtime render verifies an initially muted source against a source-removed oracle, but makes no live gate transition; BlueLive audio is inspected as CSD/catalog text, and the new independent-performance history test uses `FakeMixerGateEngine`. Exercise finite captures of running generated timeline and BlueLive performances with source/return/additive solos, mute-wins, master restoration, stateful local/downstream tails, and >256-route transitions; assert audio isolation/atomicity, transport continuity and no event retriggering. Complete the durable-action commit→undo→redo runtime matrix and record which checks used actual engines. A finite capture can explicitly stop a long-lived BlueLive session; its score duration is not evidence that audio validation is impractical. Per SC-001, SC-005, US1/AC1-6, US4/AC1-3, T016, T049, T054, T058, and T080 (partial).
- [X] T089 HIGH Finish the remaining float-render comparisons from T081 in `packages/blue-app/src/main/mixer-mute-solo.integration.test.ts` and `specs/111-mixer-mute-solo/quickstart.md`. Current real-engine pairs cover the original direct route plus shared return, soloed return, and master-muted/all-pruned output; nonzero start/end, nonconstant tempo, surviving fades/loops, global duration, extra render time, and fallback content are still covered only partially or by CSD-text assertions. Render equivalent optimized/unoptimized fixtures for those remaining cases, asserting matching float formats, exact sample counts/duration, ≤−120 dBFS residual, exact event retention/omission, and applicable sync/async parity. Preserve the improved scheduling tests instead of repeating already-proven cases per FR-012, SC-004, T039, T045, T069, T072, and T081 (partial).
- [ ] T090 HIGH Complete the outstanding T065/T082 hardware UI-to-audio acceptance on a machine with a real output/loopback device: record the documented reference configuration, both performance kinds, 100 warmed-up transitions, >256-route publication, ≤100 ms settling, transport continuity, and absence of retriggering/intermediate audible states in `specs/111-mixer-mute-solo/quickstart.md`. Existing protocol/offline tests and the recorded no-device attempt do not satisfy this criterion; keep the result explicitly outstanding when the necessary hardware is unavailable per SC-002, US1/AC6, T065, and T082 (partial).
- [X] T091 MEDIUM Finish the remaining browser interaction coverage from T083 in `packages/blue-app/src/renderer/browser/track-header-mute-solo.browser.test.tsx` and `mixer-mute-solo.browser.test.tsx`: the selector test currently collects an Event callback but never applies/publishes it, and explicitly verifies the header remains Audio; test committed Audio→Event→Audio changes with independent flags and corresponding header/strip views. Use trusted keyboard activation with resulting patch/state assertions for strip controls as well as the now-covered header mute; the strip test still dispatches a synthetic keydown and expects no activation. Include explicit Event-mode header edits and both mute/solo controls per FR-006 through FR-009, US2/AC1-6, T029, T057, and T083 (partial).
- [ ] T092 MEDIUM Complete outstanding T084 native Windows acceptance and record reproducible results in `specs/111-mixer-mute-solo/quickstart.md`: run the planned portable data, contract/history, app/engine-client, browser, and available real-engine float/gate checks using native paths, and supply the planned synthetic Windows path/error cases where absent. Current evidence still identifies only darwin arm64. State missing prerequisites and unexecuted checks explicitly rather than treating cross-platform code paths as execution evidence per plan: target-platform/host-path validation, T055, T060, and T084 (missing).
...
**Purpose**: Close the remaining runtime-history acceptance gap and carry forward the two unavailable platform measurements found by the full T001–T092 re-review. Existing task text and completion marks remain unchanged.

- [X] T093 HIGH [US4] Complete the durable mode and mixer-enable playback-reconciliation matrix through canonical `ProjectHistory` commit→undo→redo: for both running timeline and BlueLive performances, assert each action restores canonical state, independent flag sets, stable associations, dirty state, published views, and accurate applied/restart-required status through the complete sequence; include a stopped-performance counterpart and record which checks use a real engine in `packages/blue-app/src/main/project-history-mute-solo.test.ts`, `packages/blue-app/src/main/global-history-engine.integration.test.ts`, and `specs/111-mixer-mute-solo/quickstart.md`. Current mode/enable history tests verify model state and the real-engine history test verifies channel mute only, per FR-011, FR-013, SC-005, US4/AC1, T049, T054, and T088 (partial).
- [ ] T094 HIGH [US1] Complete the still-open T065/T082/T090 physical UI-to-audio acceptance on a host with an actual output/loopback device: measure 100 warmed-up timeline and BlueLive transitions in the documented 32-source/64-send setup plus a >256-route case, verify ≤100 ms audible settling without transport jumps, event retriggering, or intermediate route combinations, and record device, engine, settings, and results in `specs/111-mixer-mute-solo/quickstart.md`. The current darwin arm64 host reports no audio output device, so offline/null-backend captures cannot close this criterion per SC-002, US1/AC6, T065, T082, and T090 (partial).
- [ ] T095 MEDIUM Complete the still-open T084/T092 native Windows acceptance on a Windows host: run portable data/contract/history, app/engine-client, browser, and available real-engine float-render/gate checks with native paths; add the planned synthetic Windows embedded-path/error cases where absent; record reproducible results and any unavailable device/engine prerequisites in `specs/111-mixer-mute-solo/quickstart.md`. Current evidence is darwin arm64 only per plan: target-platform and host-path validation, T055, T060, T084, and T092 (missing).
...
**Purpose**: Carry forward the two acceptance checks still unavailable after the Phase 13 implementation pass. The T093 runtime-history matrix is implemented and recorded; existing task text and completion marks remain unchanged.

- [ ] T096 HIGH [US1] Execute the remaining physical UI-to-audio acceptance from T065/T082/T090/T094 on a machine with an actual output and loopback-capable measurement path. Record 100 warmed-up timeline and BlueLive transitions for the documented 32-source/64-send setup and a >256-route case, with measured audible settling ≤100 ms, transport continuity, no event retriggering, and no intermediate audible route combinations; document device, engine, settings, method, and results in `specs/111-mixer-mute-solo/quickstart.md`. The present host has no audio output device, so offline generated-audio and protocol timing do not satisfy SC-002 and US1/AC6 (partial).
- [ ] T097 MEDIUM Complete the native Windows acceptance still tracked by T084/T092/T095: on a Windows host, run the portable data/contract/history, app/engine-client, browser, and available real-engine float-render/gate checks using native paths; add any absent planned synthetic Windows embedded-path/error cases and record reproducible results or specific unavailable prerequisites in `specs/111-mixer-mute-solo/quickstart.md` per plan: target-platform and host-path validation, T055, and T060 (missing).

## Phase 15: Regression closure

**Purpose**: Close the user-reported M/S regressions found after the convergence pass: stale renderer edits reaching generation/render, track-header feedback that did not reflect Event-layer state, and cramped mixer-strip controls.

- [X] T098 HIGH [US1] [US2] [US3] Route native CSD-generation and disk-render commands through the renderer settlement barrier, preserve Event-layer M/S pressed state in SoundObject/track headers, keep the Score panel free of restart-only status messaging, and add horizontal padding and equal minimum sizing to mixer-strip M/S controls. Cover the canonical layer mute/unmute render path and focused renderer/browser regressions per FR-006, FR-010, FR-011, FR-012, and FR-016.

## Phase 16: UI cleanup

**Purpose**: Remove an unrequested persistent runtime status bar from the Mixer panel while preserving runtime failure handling and transient error reporting.

- [X] T099 MEDIUM [US1] Remove the persistent `RuntimeOutcomeStatus` presentation from the Mixer panel and delete its presentation-only browser coverage; keep runtime outcome reconciliation and failure toasts unchanged. This follows the product direction that persistent status bars require explicit request.

## Phase 17: Score settings relocation

**Purpose**: Move the persisted Audio/Event track-header behavior control out of Project Information
and into an extensible Score Settings modal opened from the Score toolbar.

- [X] T100 MEDIUM [US2] Add focused renderer coverage for the Score Settings modal: Audio/Event
  selection, saved-versus-effective and mixer-disabled explanations, legacy compatibility notice,
  accessible dialog controls, and the absence of the selector from Project Information in
  `packages/blue-app/src/renderer/tests/score-settings-dialog.test.tsx` and
  `packages/blue-app/src/renderer/tests/project-properties-mute-solo.test.tsx`.
- [X] T101 MEDIUM [US2] Add the far-right Score toolbar gear beside Ruler and implement the
  extensible Score Settings modal in
  `packages/blue-app/src/renderer/components/workbench/panels/score/ScoreToolbar.tsx`,
  `packages/blue-app/src/renderer/components/workbench/panels/ScorePanel.tsx`, and
  `packages/blue-app/src/renderer/components/workbench/panels/score/ScoreSettingsDialog.tsx`;
  route mode changes through the existing semantic ProjectHistory action and remove Audio/Event
  controls and mode diagnostics from `ProjectInformationTab.tsx`.
- [X] T102 MEDIUM [US2] Update the feature's spec, UI contract, implementation plan, and
  validation quickstart to document Score Settings as the mode-control location and Project
  Information as metadata-only; run the focused renderer tests, app build, lint, and whitespace
  validation per FR-007 through FR-009 and the Score settings access requirements.

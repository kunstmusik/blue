...
- **Intentional Divergences**: New-project track headers default to audio behavior; mixer channel flags become audible; audio controls work during playback. Legacy nonzero channel flags may change the mix and require the FR-015 notice. Java applications are not expected to reproduce these new audio semantics.
- **State Ownership**: The active BlueData project owns track event flags, channel audio flags, mixer enable state, and the header preference, persisted in .blue XML. Derived solo routing, UI indicators, and playback-applied status are disposable. CSD/audio exports are derived artifacts, not project stores. No new app-wide preference is introduced.

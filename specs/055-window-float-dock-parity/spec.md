...
5. **Given** a view/auxiliary tab is docked, **When** the context menu opens, **Then** Close Group, Minimize, Minimize Group, Float, Float Group, Dock, Dock Group, Move, Shift Left, Shift Right, Move Group, and Size Group are represented with state-dependent enablement; Move and Move Group expose left, right, and bottom edge targets, and Size Group exposes larger, smaller, and reset actions.
...
- **FR-014**: The view/auxiliary tab context menu MUST include user-facing commands for Close, Close Group, Maximize or Restore as appropriate, Minimize, Minimize Group, Float, Float Group, Dock, Dock Group, Move, Shift Left, Shift Right, Move Group, and Size Group. For docked auxiliary panels, Move and Move Group MUST provide left, right, and bottom edge targets, transfer the selected panel(s) into the target edge group, and leave the seeded default mode unchanged for later reveals. Size Group MUST provide larger, smaller, and reset actions.
...
- **FR-029**: Editor-mode panels, output-mode panels, properties panels, and auxiliary panels MUST retain their Java Blue-inspired default modes after Move, Move Group, docking, or reset; moving content MUST NOT relocate the default mode anchor itself.
...
- The required first-slice context-menu command set is the NetBeans tab popup set for editor and view modes. Move and Move Group transfer auxiliary panel(s) into derived target-edge groups while leaving seeded default-mode anchors stable; Size Group is implemented for docked auxiliary groups through controlled-pane-size subcommands.

...
**Constraints**: All-or-nothing commit preserved; shared `javaScriptSession` never used concurrently; single active render/freeze operation; at most 32 concurrent jobs; existing IPC status channels and event shape remain compatible, with an additive `rendered` item phase

**Scale/Scope**: `freeze-score-objects.ts` executor, `program-settings.ts` + usage matrix, the shared freeze status contract, `UtilitySettings.tsx`, the freeze-operation store/dialog, and their tests; no `@blue/data` changes, no new packages
...
- **Canonical ownership and contracts**: PASS — `utility.freezeMaxJobs` is owned by the main-process program-settings store (app-wide JSON, merge-with-defaults plus normalization on load); canonical score mutation remains exclusively in the main-process freeze orchestration. No new IPC channels: the existing `RENDER_OPERATION_STATUS_CHANNEL` and `FREEZE_ITEM_STATUS_CHANNEL` events are reused with a shape-compatible additive `rendered` item phase and renderer-derived focus state. The executor seam's return type gains a typed optional `errorCode` — a main-internal contract documented in `contracts/`.
...
**Post-design re-check**: PASS — the Phase 1 design adds one settings field (normalized, validated, default-filled), one main-internal seam extension (`errorCode`), and additive renderer item-phase/focus state while preserving the existing channels and event shape. No new persistence store, IPC channel, data-core surface, runtime boundary, or path handling is introduced. No constitution exception is required.
...
│   ├── render-freeze-contract.ts       # additive rendered item phase; existing event shape/channel
│   └── program-settings.test.ts
└── renderer/
    ├── components/settings/UtilitySettings.tsx   # numeric field, min 1 / max 32, default 4
    ├── stores/freeze-operation-store.ts          # concurrent row lifecycle and stable output focus

...
**Implementation**: `develop` at `bf2cf17` (2026-08-22)
...
- What happens with a hand-edited settings file containing an extreme or malformed value? Loading normalizes it to the default of 4 rather than starting an excessive number of renders.
...
- **FR-009**: The operation MUST report a single aggregate progress across all jobs that is independent of job completion order and reaches 100% only on successful completion; the status events consumed by the renderer MUST remain shape-compatible with the existing render/freeze events so existing UI continues to function. Additive item phases, including `rendered`, and renderer-derived row state are allowed, but no new IPC channel or incompatible event shape may be introduced.
...
- **SC-007 (post-launch outcome metric)**: Users freezing multiple CPU-bound objects wait materially less than the sum of sequential render times at the default setting on a multi-core machine. The feature validation record establishes overlap and cap enforcement; a controlled before/after wall-clock benchmark is product-performance evidence rather than a build-time correctness gate.
...
- The renderer freeze UI (busy state, toast, progress) continues to consume the existing status channels and event shape; the per-item `rendered` phase and stable focused-row behavior support parallel interleaving without adding a channel.
